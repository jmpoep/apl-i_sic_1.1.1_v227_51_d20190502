...
#if INSTRUMENTALGOS
  static UINT64 LCTJedecResetCount=1;
  static UINT64 LCTJedecResetTimeTotal=0;
#endif //INSTRUMENTALGOS
  ...
  )
{
  if (MrcData->HalfClkPi==0)
  {
    if (MrcData->DigitalDll < 2) {
      return HalfClk [ADdll [MrcData->DigitalDll]][MrcData->Channel[Channel].CurrentFrequency];
    } else {
      return 0;
    }
  }
  ...
  )
{
  ...
  for (Channel = 0; Channel < MAX_CHANNELS; Channel++) {
    for (i = 0; i < 16; i++) {
      if ((MrcData->Channel[Channel].CurrentPlatform & (1 << i)) != 0) {
        MrcData->Channel[Channel].CurrentPlatform = i;
      }
      if ((MrcData->Channel[Channel].CurrentFrequency & (1 << i)) != 0) {
        MrcData->Channel[Channel].CurrentFrequency = i;
      }
      if ((MrcData->Channel[Channel].CurrentConfiguration & (1 << i)) != 0) {
        MrcData->Channel[Channel].CurrentConfiguration = i;
      }
      if ((MrcData->Channel[Channel].CurrentDdrType & (1 << i)) != 0) {
        MrcData->Channel[Channel].CurrentDdrType = i;
      }
    }
  }
}
  ...
  )
{
  ...
  for (Channel = 0; Channel < MAX_CHANNELS; Channel++) {
    if (((j->PList & (1 << MrcData->Channel[Channel].CurrentPlatform))      != 0) &&
        ((j->FList & (1 << MrcData->Channel[Channel].CurrentFrequency))     != 0) &&
        ((j->CList & (1 << MrcData->Channel[Channel].CurrentConfiguration)) != 0) &&
        ((j->TList & (1 << MrcData->Channel[Channel].CurrentDdrType))       != 0)
      )
    {
      MrcData->Channel[Channel].ExecuteOnThisChannel = TRUE;
    } else {
      MrcData->Channel[Channel].ExecuteOnThisChannel = FALSE;
    }
  }
}
      ...
      for (j; j->Function != NULL; j++) {
        ...
        if ((MrcData->BootMode & j->BootMode) != 0) {
          ...
          if ((j->Channel == ChAll) || (j->Channel == ChAllP && !MrcData->EnableParallelTraining)) {
            for (Channel = 0; Channel < MAX_CHANNELS; Channel++) {
              if (!MrcData->Channel[Channel].Enabled || !MrcData->Channel[Channel].ExecuteOnThisChannel) {
                continue;
              }
            ...
            //
            MrcData->Channel[0].ExecuteOnThisChannel = TRUE;
            MrcData->ExecuteThisRoutineInParallel = FALSE;
            Status = (*(j->Function)) (MrcData, j->CapsuleStartIndex, j->StringIndex, 0);
            if (Status != MMRC_SUCCESS) {
              return Status;
            ...
            if (j->Channel < MAX_CHANNELS) {
              if (MrcData->Channel[j->Channel].Enabled && MrcData->Channel[j->Channel].ExecuteOnThisChannel) {
                MrcData->ExecuteThisRoutineInParallel = FALSE;
  ...
  )
{
  ...
  for (Channel; Channel < MAX_CHANNELS; Channel++) {
    for (Rank = 0; Rank < MAX_RANKS; Rank++) {
      ...
      //
      MrcData->Channel[Channel].FaultyPart[Rank].Byte0to3 = (UINT32)(~(0));
      MrcData->Channel[Channel].FaultyPart[Rank].Byte4to7 = (UINT32)(~(0));
      MrcData->Channel[Channel].FaultyPart[Rank].ByteEcc = (UINT8)(~(0));
      //
      // Initialize the cumulative variables for historic error tracling across all trainings
      //
      if (FirstTraining) {
        MrcData->Channel[Channel].FaultyPart[Rank].DataBitFailCumulativeHigh = 0;
        MrcData->Channel[Channel].FaultyPart[Rank].DataBitFailCumulativeLow = 0;
        MrcData->Channel[Channel].FaultyPart[Rank].EccBitFailCumulative = 0;
      }
    }
  ...
  )
{
  ...
  if (MrcData->Channel[Channel].MaxDq[Rank] <= MAX_STROBE_X8_DEVICES) {
  ...
  )
{
  ...
  if (MrcData->FaultyPartTracking && MrcData->RestorePath == FALSE) {
    for (Rank = 0; Rank < MAX_RANKS; Rank++) {
      if (MrcData->Channel[Channel].RankEnabled[Rank]) {
        //
        // Failures on the current training and previews trainings
        //
        FaultyPartRankLow = MrcData->Channel[Channel].FaultyPart[Rank].Byte0to3 | MrcData->Channel[Channel].FaultyPart[Rank].DataBitFailCumulativeLow;
        FaultyPartRankHi  = MrcData->Channel[Channel].FaultyPart[Rank].Byte4to7 | MrcData->Channel[Channel].FaultyPart[Rank].DataBitFailCumulativeHigh;
        FaultyPartRankEcc = MrcData->Channel[Channel].FaultyPart[Rank].ByteEcc | MrcData->Channel[Channel].FaultyPart[Rank].EccBitFailCumulative;

        if (FaultyPartRankLow != 0 || FaultyPartRankHi != 0 || FaultyPartRankEcc != 0) {
          //
          // Acumulate failures through out all trainings
          //
          MrcData->Channel[Channel].FaultyPart[Rank].DataBitFailCumulativeLow = FaultyPartRankLow;
          MrcData->Channel[Channel].FaultyPart[Rank].DataBitFailCumulativeHigh = FaultyPartRankHi;
          MrcData->Channel[Channel].FaultyPart[Rank].EccBitFailCumulative  = FaultyPartRankEcc;

          if (((CountOnes(FaultyPartRankLow) + CountOnes(FaultyPartRankHi)) > FAULTY_PART_CORRECTABLE) || (FaultyPartRankEcc != 0)) {
            MrcData->Channel[Channel].FaultyPart[Rank].ErrorType = FAULTY_PART_UNCORRECTABLE;
          } else {
            MrcData->Channel[Channel].FaultyPart[Rank].ErrorType = FAULTY_PART_CORRECTABLE;
          }
          //
          // Display the error on serial log
          //
          if (MrcData->Channel[Channel].FaultyPart[Rank].ErrorType == FAULTY_PART_UNCORRECTABLE) {
          ...
          FaultyPartProcessDeviceData (MrcData, Channel, Rank, FaultyPartRankLow, FaultyPartRankHi, FaultyPartRankEcc);
          if (MrcData->Channel[Channel].Enabled == 0) {
  ...
  if (MrcData->FaultyPartTracking) {
    ...
    for (Strobe = 0; Strobe < MAX_STROBES; Strobe++) {
      ...
      if ((Strobe < LowCrsLimit)) {
        ...
        StrobeBitLanes2 = ((MrcData->Channel[Channel].FaultyPart[Rank].Byte0to3 >> (StrobeBits * Strobe)) & StrobeMask);
        //
        ...
        if (CountOnes (StrobeBitLanes) < CountOnes (StrobeBitLanes2)) {
          MrcData->Channel[Channel].FaultyPart[Rank].Byte0to3 &= (UINT32) ~(StrobeMask << (StrobeBits * Strobe));
          MrcData->Channel[Channel].FaultyPart[Rank].Byte0to3 |= (UINT32) ((StrobeBitLanes << (StrobeBits * Strobe)) & (StrobeMask << (StrobeBits * Strobe)));
        }
      }
      if ((Strobe > HighCsrLimit) && (Strobe < EccCsrLimit)) {
        StrobeBitLanes = ((Cpgc_Err_Stat_Hi >> (StrobeBits * (Strobe - LowCrsLimit))) & StrobeMask);
        StrobeBitLanes2 = ((MrcData->Channel[Channel].FaultyPart[Rank].Byte4to7 >> (StrobeBits * (Strobe - LowCrsLimit))) & StrobeMask);
        if (CountOnes (StrobeBitLanes) < CountOnes (StrobeBitLanes2)) {
          MrcData->Channel[Channel].FaultyPart[Rank].Byte4to7 &= (UINT32) ~(StrobeMask << (StrobeBits * (Strobe - LowCrsLimit)));
          MrcData->Channel[Channel].FaultyPart[Rank].Byte4to7 |= (UINT32) ((StrobeBitLanes << (StrobeBits * (Strobe - LowCrsLimit))) & (StrobeMask << (StrobeBits * (Strobe - LowCrsLimit))));
        }
      }
      if ((Strobe >= EccCsrLimit)) {
        StrobeBitLanes = ((Ecc_lane_err_stat >> (StrobeBits * (Strobe - EccCsrLimit))) & StrobeMask);
        StrobeBitLanes2 = (MrcData->Channel[Channel].FaultyPart[Rank].ByteEcc & StrobeMask);
        if (CountOnes (StrobeBitLanes) < CountOnes (StrobeBitLanes2)) {
          MrcData->Channel[Channel].FaultyPart[Rank].ByteEcc &= (UINT32) ~(StrobeMask << (StrobeBits * (Strobe - EccCsrLimit)));
          MrcData->Channel[Channel].FaultyPart[Rank].ByteEcc |= (UINT32) ((StrobeBitLanes << (StrobeBits * Strobe)) & (StrobeMask << (StrobeBits * Strobe)));
        }
      }
    } // for strobe
    //
    ...
    if (((CountOnes (Cpgc_Err_Stat_Lo) + CountOnes(Cpgc_Err_Stat_Hi)) == 1) && (CountOnes (Ecc_lane_err_stat) == 0)
          && (CountOnes (Bytegrp_err_stat) == 1) && MrcData->Channel[Channel].EccEnabled) {
  ...
  )
{
  ...
  for (Channel = 0; Channel < MAX_CHANNELS; Channel++) {
    ...
    for (Dimm = 0; Dimm < MAX_DIMMS; Dimm++) {
      if (!MrcData->Channel[Channel].DimmPresent[Dimm]) {
        continue;
      }
      for (Rank = 0; Rank < MAX_RANKS_PER_DIMM; Rank++) {
        if (MrcData->Channel[Channel].RankEnabled[(2 * Dimm) + Rank] == 0) {
          continue;
        }
        switch (MrcData->Channel[Channel].D_DataWidth[Dimm]) {
        case 0:
          MrcData->Channel[Channel].MaxDq[(2 * Dimm) + Rank] = 18;
          break;
        case 1:
          MrcData->Channel[Channel].MaxDq[(2 * Dimm) + Rank] = 9;
          break;
        case 2:
          MrcData->Channel[Channel].MaxDq[(2 * Dimm) + Rank] = 4;
          break;
        }
      } // Rank
    } // Dimm
  ...
  )
{
  ...
  if (Entry == MMRC_ENTRY) {
    ...
    //
    if (MrcData->Channel[Channel].DramType == TypeDdr4) {
...
#if LPDDR4_SUPPORT
   //
   // Reset BL = 32-bit mode.
   //
    /*
   if (MrcData->Channel[Channel].DramType == TypeLpDdr4) {
     MrcData->DynamicVars[Channel][REG_DATA] = 0;
  ...
  */
  ...
  for (Strobe = 0; Strobe < MrcData->Channel[Channel].MaxDq[0]; Strobe++) {
    for (Bit = 0 ; Bit < MAX_BITS; Bit++) {
      TempValue = 7;
      GetSetDdrioGroup (MrcData, 0, Channel, 0, 0, Strobe, 0, MrcData->DVFSFreqIndex, 0xFF, RxVocVal0+Bit, CMD_SET_VAL, &TempValue);
    }
  }
  //
  // Set the COMP Vref for the Pull up Ron and issue the Init.
  //
  if (LdoEnable == TRUE && MrcData->Channel[Channel].DramType == TypeLpDdr4) {
...
#endif
#if DDR3_SUPPORT==1
  } else if (MrcData->Channel[Channel].DramType == TypeDdr3All) { // DDR3L
  ...
  MmrcExecuteTask (MrcData, VOC_COMP_VREF_AND_INIT, NO_PRINT, 0);
  //
  ...
  for (LoopIteration = 0; LoopIteration < 2; LoopIteration++) {
    if (LoopIteration == 0) {
      ...
      for (Strobe = 0; Strobe < MrcData->Channel[Channel].MaxDq[0]; Strobe++) {
        ...
        if ((Strobe %4 == 3) && (Strobe < (MrcData->Channel[Channel].MaxDq[0] -1)))
          MmrcDebugPrint ((MMRC_DBG_MIN, "\n     ",TempValue));
        else
    ...
    Create1DSweepLastPass (MrcData, Channel, 0, RxVref, 1, Dim1StartPoint, Low, High, VOC_VREF_STEP_SIZE,
       FALSE, FALSE, TRUE, MrcData->Channel[Channel].MaxDq[0], VrefValueToSwitchVOC, Results, "S", 0);
  ...
  )
{
  ...
  if (MrcData->Channel[Channel].DramType == TypeLpDdr4) {
    InitialDelay  = RCVN_INITIAL2XVAL_LP4 - 2;
#if LPDDR3_SUPPORT==1
  } else if (MrcData->Channel[Channel].DramType == TypeLpDdr3){
    InitialDelay  = RCVN_INITIAL2XVAL_LP3;
#endif
#if DDR3_SUPPORT==1
  } else if (MrcData->Channel[Channel].DramType <= TypeDdr3All){
    InitialDelay  = RCVN_INITIAL2XVAL_DDR3;
#endif
  }
  ...
  for (Rank = 0; Rank < MAX_RANKS; Rank++) {
    ...
    Create1DSweepLastPass (MrcData, Channel, Rank, RecEnDelay, 1, Dim1StartPoint, Low, High, LargeStep,
      FALSE, FALSE, FALSE, MrcData->Channel[Channel].MaxDq[Rank], SampleDqs, Results, "S", 0);
    //
    ...
    Create1DSweepLastPass (MrcData, Channel, Rank, RecEnDelay, 1, Dim1StartPoint, Low, High, SmallStep,
      FALSE, FALSE, FALSE, MrcData->Channel[Channel].MaxDq[Rank], SampleDqs, Results, "S", DitherVal);
    //
    ...
    //
    SampleDqs(MrcData, Channel, Rank, MrcData->Channel[Channel].MaxDq[Rank], 1, PassFail);
    for (Strobe = 0; Strobe < MrcData->Channel[Channel].MaxDq[Rank]; Strobe++) {
  ...
  )
{
  ...
  //
  TotalFinishStrobes = (1 << MrcData->Channel[Channel].MaxDq[Rank]) - 1;
  TempValue = 0;
  ...
  )
{
  ...
  if (Cmd & RD_ONLY) {
    ...
    case TypeCache:
      *Value = (UINT32) MrcData->CachedValues[Channel][CacheIndex][Rank][Strobe];
      break;
    case TypeCachePB:
      *Value = (UINT32) MrcData->CachedValuesPB[Channel][CacheIndex][Rank][Strobe][Bit];
      break;
    case TypeCacheNonSR:
      *Value = (UINT32) MrcData->CachedValuesNonSR[Channel][CacheIndex][Rank][Strobe];
      break;
    case TypeCacheNonSRPB:
      *Value = (UINT32) MrcData->CachedValuesNonSRPB[Channel][CacheIndex][Rank][Strobe][Bit];
      break;
    default:
      MrcDeadLoop ();
      break;
    ...
    switch (CacheArrayIndex) {
    case TypeCache:
      MrcData->CachedValues[Channel][CacheIndex][Rank][Strobe] = *Value;
      break;
    case TypeCachePB:
      MrcData->CachedValuesPB[Channel][CacheIndex][Rank][Strobe][Bit] = *Value;
      break;
    case TypeCacheNonSR:
  ...
  if (Type < ALGO_REG_INDEX) {
    ...
    if ((Type - ALGO_REG_INDEX) < MAX_SIGNAL_INFO_ELEMENTS) {
      if (FrequencyIndex < SignalInfo[Type - ALGO_REG_INDEX].FrequencyOffset) {
        ...
        //
        if ((Cmd & RD_ONLY) != 0 && (Cmd & RD_REG) == 0) {
          *Value = MrcData->Channel[Channel].Trained_Value.Values[MrcData->CurrentBlueprint][(Type - ALGO_REG_INDEX) + FrequencyIndex][Rank][Strobe];
        }
        //
        // If a cache write, then write directly to the trained values array.
        //
        if (Cmd & UPD_CACHE) {
          MrcData->Channel[Channel].Trained_Value.Values[MrcData->CurrentBlueprint][(Type - ALGO_REG_INDEX) + FrequencyIndex][Rank][Strobe] = (UINT16) NewValue;
        }
      } else {
        MrcData->CurrentBlueprint = TempBlueprint;
        return STATUS_FAIL;//MrcFrequencyIndexNotSupported;
  ...
  )
{
  ...
  for (ElementIndex = 0; ElementIndex < NumberOfDelayElementsPerAlgo; ElementIndex++) {
    ...
    //
    ElementValue = CurrentValue / (MrcData->GranularitySteps[MrcData->CurrentBlueprint][ADdll[MrcData->DigitalDll]][MrcData->Channel[Channel].CurrentFrequency][ElementIndex]);
    TempValue = ElementValue * (MrcData->GranularitySteps[MrcData->CurrentBlueprint][ADdll[MrcData->DigitalDll]][MrcData->Channel[Channel].CurrentFrequency][ElementIndex]);
    //
    // We cannot set any delay larger than the 2x clock. If the delay element is larger granularity, skip it.
    //
    if (MrcData->GranularitySteps[MrcData->CurrentBlueprint][ADdll[MrcData->DigitalDll]][MrcData->Channel[Channel].CurrentFrequency][ElementIndex] > GetHalfClk (MrcData, Channel))
    {
      continue;
    }
    //
  ...
  *Value = 0;
  ...
  for (ElementIndex = 0; ElementIndex < NumberOfDelayElementsPerAlgo; ElementIndex++) {
    ...
    if (GetSetDdrioGroup (MrcData, Socket, Channel, Dimm, Rank, Strobe, Bit, FrequencyIndex, IoLevel, AlgoElementsStart + ElementIndex, Cmd, &TempValue) == MMRC_SUCCESS) {
      if ((MrcData->CurrentBlueprint < MAX_BLUEPRINTS) && (MrcData->DigitalDll < 2)) {
        *Value += TempValue * (MrcData->GranularitySteps[MrcData->CurrentBlueprint][ADdll[MrcData->DigitalDll]][MrcData->Channel[Channel].CurrentFrequency][ElementIndex]);
      }
    }
    if (ElementIndex >= NumberOfDelayElementsPerAlgo - NumberOfDependantElements) {
      if ((MrcData->CurrentBlueprint < MAX_BLUEPRINTS) && (MrcData->DigitalDll < 2)) {
        CCValue += (UINT16) TempValue * (MrcData->GranularitySteps[MrcData->CurrentBlueprint][ADdll[MrcData->DigitalDll]][MrcData->Channel[Channel].CurrentFrequency][ElementIndex]);
      }
    }
  } // ElementIndex loop ...
  ...
  for (ElementIndex = 0; ElementIndex < NumberOfCCElements; ElementIndex++) {
    ...
    if (Elements[AlgoElementsStart + ElementIndex].ElementAttributes.Bits.GranularityValid == 1) {
      ...
      *Value -= TempValue * (MrcData->GranularitySteps[MrcData->CurrentBlueprint][ADdll[MrcData->DigitalDll]][MrcData->Channel[Channel].CurrentFrequency][Elements[AlgoElementsStart + ElementIndex].ElementAttributes.Bits.GranularityValid]);
    }
  } // ElementIndex loop ...
#endif
  return MMRC_SUCCESS;
    ...
    for (ElementIndex = 0; ElementIndex < NumberOfDelayElementsPerAlgo; ElementIndex++) {
      ...
      if ( (AlgoElementsStart + ElementIndex) < MAX_NUM_ELEMENTS) {
        if (Elements[AlgoElementsStart + ElementIndex].CacheIndex != 0xFF) {
          ...
          *MaxLimit += TempLimit * MrcData->GranularitySteps[MrcData->CurrentBlueprint][ADdll[MrcData->DigitalDll]][MrcData->Channel[Channel].CurrentFrequency][ElementIndex];
        }
      }
    } // ElementIndex loop ...
  } // if (Type < ALGO_REG_INDEX)
...
#endif
  if ((MrcData->CpuStepping == STEPPING_E0) || (MrcData->CpuStepping == STEPPING_E1)) {
    MrcDelay (MrcData, MICRO_DEL, 1000);    // Temp WA to deal with RCVEN failures on BXT-B
  }
...
#endif // LPDDR4_SUPPORT
#if DDR3_SUPPORT
    if (MrcData->Channel[Channel].DramType <= TypeDdr3All) {
      for (ConfigureRank = 0; ConfigureRank < MAX_RANKS; ConfigureRank++) {
        if (!MrcData->Channel[Channel].RankEnabled[ConfigureRank]) {
          continue;
        }
  ...
  )
{
...
#if (DDR3_SUPPORT==1)
  if (MrcData->Channel[Channel].OdtHigh) {
  ...
  Create1DSweepLastPass(MrcData, Channel, Rank, TxDqsDelay, 1, Dim1StartPoint, Low, High, SmallStep,
    FALSE, FALSE, FALSE, MrcData->Channel[Channel].MaxDq[Rank], SampleDqs, Results, "S", 3);
  ...
  )
{
...
#endif
  } else { // MrcData->Channel[Channel].DramType == TypeLpDdr3
#if LPDDR3_SUPPORT==1
  ...
  for (Rank = 0; Rank < MAX_RANKS; Rank++) {
    ...
    Create1DSweepLastPass(MrcData, Channel, Rank, RxDqsDelay, 1, Dim1StartPoint, Low, High, LargeStep,
     FALSE, FALSE, FALSE, MrcData->Channel[Channel].MaxDq[Rank], EarlyReadTest, Results, "S", 0);
    ...
    Create1DSweepLastPass(MrcData, Channel, Rank, RxDqsDelay, 1, Dim1StartPoint, Low, High, SmallStep,
     FALSE, FALSE, FALSE, MrcData->Channel[Channel].MaxDq[Rank], EarlyReadTest, Results, "S", DitherVal);
    //
  ...
  // Print out or restore the training results.
  //
  DisplayOrRestoreTrainingResults(MrcData, Channel, RxDqsPDelay, TRUE);
  DisplayOrRestoreTrainingResults(MrcData, Channel, RxDqsNDelay, TRUE);
  //
  ...
  )
{
 ...
 //        MmrcExecuteTask(MrcData, MRC_to_Tap_CWL, NO_PRINT, Channel);
    ...
    Create1DSweepLastPass(MrcData, Channel, Rank, TxDqsDelay, 1, Dim1StartPoint, Low, High, 2 * GetHalfClk(MrcData, Channel),
      FALSE, TRUE, FALSE, MrcData->Channel[Channel].MaxDq[Rank], CoarseWLTest, Results, "S", 0);
  ...
  )
{
  ...
  //
  if ((MrcData->Channel[Channel].EccStrobes & (1 << Strobe)) != 0) {
    return TRUE;
  }
  return FALSE;
}
  ...
  )
{
  ...
  //
  if ((MrcData->Channel[Channel].RankEnabled[Rank] == FALSE) ||
      (Strobe >= MrcData->Channel[Channel].MaxDq[Rank] ||
      (!MrcData->Channel[Channel].EccEnabled && (IsEccStrobe (MrcData, Channel, Strobe))))) {
    return FALSE;
  } else {
    return TRUE;
  }
...
)
{
  ...
  if (MrcData->Channel[Channel].DramType == TypeLpDdr4 || MrcData->Channel[Channel].DramType <= TypeDdr3All || MrcData->Channel[Channel].DramType == TypeLpDdr3) {
    //
    ...
    for (Element = 0; Element < NumberElements; Element++) {
      ...
      if (MrcData->Channel[Channel].DramType == TypeLpDdr4 || MrcData->Channel[Channel].DramType == TypeLpDdr3) {
      ...
      for (Knob=0; Knob < MAX_KNOBS; Knob++) {
        ...
        */
        Dim1Min[Channel][Element]                      = (UINT16) (DQValue - GetQtrClk (MrcData, Channel)/2 + 320/PiInPs);
        Dim1Max[Channel][Element]                      = (UINT16) (DQValue + GetQtrClk (MrcData, Channel) + 320/PiInPs);
        Dim1StartPoint[Channel][Knob][Element][LOW]    = (UINT16) (DQValue - GetQtrClk (MrcData, Channel)/2 + 320/PiInPs);
        Dim1StartPoint[Channel][Knob][Element][HIGH]   = (UINT16) (DQValue + GetQtrClk (MrcData, Channel) + 320/PiInPs);
#else
        if (MrcData->Channel[Channel].DramType == TypeLpDdr4 ) {
          Dim1Min[Channel][Element]                      = (UINT16) (DQValue - GetQtrClk(MrcData, Channel));    //     Used to add "+200 / PiInPs", but saw issues at 3200 with Hynix where it clipped lowside passing region
          Dim1Max[Channel][Element]                      = (UINT16) (DQValue + GetQtrClk (MrcData, Channel) + 800/PiInPs);
          Dim1StartPoint[Channel][Knob][Element][LOW]    = (UINT16) (DQValue - GetQtrClk(MrcData, Channel));   //     Used to add "+ 200/PiInPs"
    ...
    Create1DSweepLastPass (MrcData, Channel, Rank, TxDqDelay, 1, Dim1StartPoint, Dim1Min, Dim1Max, CWL_DQ_STEP, FALSE, FALSE, FALSE, MrcData->Channel[Channel].MaxDq[Rank], GenericByteMemoryTest, Results, "", 5);
    AllPassFlag = TRUE;
    for (Element=0; Element < NumberElements; Element++) {
...
)
{
  ...
  // The maximum number of grpup cnt is 2 based on the SignalInfoGenerated[MAX_BLUEPRINTS][MAX_SIGNAL_INFO_ELEMENTS]
  ...
  for (Channel = ChannelBackup; Channel < MAX_CHANNELS; Channel++) {
    ...
    if (MrcData->Channel[Channel].EccEnabled) {
    ...
    else
    {
      ...
      DevWidth = MAX_BITS; //0x1 << (MrcData->Channel[Channel].D_BusWidth[0] + 2); //  PT Assume dimm slot 0 is always populated. Should use rank to find the dimm index later
  ...
  if ( FindErrorAtAnyChannel ) {
    ...
    for (Channel = ChannelBackup; Channel < MAX_CHANNELS; Channel++) {
      ...
      if (MrcData->CpgcOptions.RmtRestoreJedecResetType & RMT_FAIL_JEDEC_RESET) {
        ...
        CompleteJedecInit(MrcData, 0, 0, Channel);
        MrcData->ExecuteThisRoutineInParallel=TRUE;
        break;  // Only need to call JEDEC reset on one of the channel
      }
    } // Channel
  ...
  )
{
  ...
  for (Index = InitIndex; Index < MaxIndex; Index++) {
    ...
    for (Rank = 0; Rank < MAX_RANKS; Rank++) {
      ...
      DeltaT[ExecuteRead]   = MrcData->Channel[Channel].RMT_Data[Rank][RxDqRight] - MrcData->Channel[Channel].RMT_Data[Rank][RxDqLeft];
      DeltaV[ExecuteRead]   = MrcData->Channel[Channel].RMT_Data[Rank][RxVHigh]   - MrcData->Channel[Channel].RMT_Data[Rank][RxVLow];
      DeltaT[ExecuteWrite]  = MrcData->Channel[Channel].RMT_Data[Rank][TxDqRight] - MrcData->Channel[Channel].RMT_Data[Rank][TxDqLeft];
      DeltaV[ExecuteWrite]  = MrcData->Channel[Channel].RMT_Data[Rank][TxVHigh]   - MrcData->Channel[Channel].RMT_Data[Rank][TxVLow];
  ...
  )
{
  ...
  CONTROLKNOBS *ControlKnobs = NULL;
  ...
  //
  //MrcData->Channel[1].Enabled = FALSE;
  MrcData->MaxElements = 0;
  for (ch=0; ch < MAX_CHANNELS; ch++) {
    for (rk=0; rk < MAX_RANKS; rk++) {
      if (RunOnThisChannel (MrcData, ch, rk)) {
        MrcData->MaxElements = MAX(MrcData->MaxElements, MrcData->Channel[ch].MaxDq[rk]);
      }
    }
  }
  for (i=0; i< RT_NUM_EYEMASKS; i++)
  {
    ...
    LocalEyeMaskRead[i].MinimumDelayWidth=(EyeMaskPctPiRd[MrcData->Channel[0].CurrentDdrType][MrcData->CurrentFrequency][i] * GetHalfClk(MrcData, 0))/100;
  ...
  )
{
  ...
  //UINT32 LocalOdt=ODT;
  UINT32 LocalOdt=DramOdt[MrcData->Channel[0].CurrentDdrType][MrcData->Channel[0].CurrentFrequency];
  //UINT32 WrVrefOvr = WT_VREFOVERRIDE_VALUE;
  UINT16 WrVrefOvr = DramVrefOvr[MrcData->Channel[0].CurrentDdrType][MrcData->Channel[0].CurrentFrequency];
  ...
  for (Ch = 0; Ch < MAX_CHANNELS; Ch++) {
    for (Rank = 0; Rank < MAX_RANKS; Rank++) {
      if (RunOnThisChannel (MrcData, Ch, Rank)) {
        MrcData->MaxElements = MAX(MrcData->MaxElements, MrcData->Channel[Ch].MaxDq[Rank]);
      }
    }
  }
  for (i=0; i< WT_NUM_EYEMASKS; i++)
  {
    ...
    LocalEyeMaskWrite[i].MinimumDelayWidth=(EyeMaskPctPiWr[MrcData->Channel[0].CurrentDdrType][MrcData->CurrentFrequency][i] * GetHalfClk(MrcData, 0))/100;
  ...
  // Print out or restore the training results.
  //
  DisplayOrRestoreTrainingResults (MrcData, Channel, TxDqDelay_Actual, TRUE);
  //
  // End with the exit task for write training.
  //
...
)
{
  ...
  for (Strobe = 0; Strobe < MrcData->Channel[Channel].MaxDq[Rank]; Strobe++) {
    ...
    for (Bit = 0; Bit < MAX_BITS; Bit++) {
      ...
      MrcData->Channel[Channel].RMT_DQData[Rank][RmtTableIndex][Bit + Strobe * MAX_BITS][LOW] = rmtBlMargin;
      //
      // Read Left or Low margin
      //
      if ( rmtBlMargin > RmtMinLeftMargin) {
        MrcData->Channel[Channel].RMT_Data[Rank][RmtTableIndex][LOW] = rmtBlMargin;
      ...
      if ( rmtBlMargin < 0 ) rmtBlMargin = 0;
      MrcData->Channel[Channel].RMT_DQData[Rank][RmtTableIndex][Bit + Strobe * MAX_BITS][HIGH] = rmtBlMargin;
      //
      // Read Right or High margin
      //
      if (rmtBlMargin < RmtMinRightMargin) {
        MrcData->Channel[Channel].RMT_Data[Rank][RmtTableIndex][HIGH] = rmtBlMargin;
    ...
    if (MrcData->Channel[Channel].RMT_Data[Rank][RmtTableIndex][LOW] > 0)
      MrcData->Channel[Channel].RMT_Data[Rank][RmtTableIndex][LOW] = 0;
    if (MrcData->Channel[Channel].RMT_Data[Rank][RmtTableIndex][HIGH] < 0)
      MrcData->Channel[Channel].RMT_Data[Rank][RmtTableIndex][HIGH] = 0;
#endif
#if RMT_DEBUG
    MmrcDebugPrint ((MMRC_DBG_DEBUG, " %02d:%02d   ", MrcData->Channel[Channel].RMT_Data[Rank][RmtTableIndex][LOW], MrcData->Channel[Channel].RMT_Data[Rank][RmtTableIndex][HIGH]));
#endif
  ...
  for (Strobe = 1; Strobe < MrcData->Channel[Channel].MaxDq[Rank]; Strobe++) {
    StrobeTemp = Strobe;
    BreakStrobeLoop (MrcData,  Channel, Rank, DelaySignal, &StrobeTemp);

    if (StrobeTemp != Strobe) { // Cmd, Ctl TxvRef, etc
      for (Bit = 0; Bit < MAX_BITS; Bit++) {
        MrcData->Channel[Channel].RMT_DQData[Rank][RmtTableIndex][Bit + Strobe * MAX_BITS][LOW] = MrcData->Channel[Channel].RMT_DQData[Rank][RmtTableIndex][Bit ][LOW];
        MrcData->Channel[Channel].RMT_DQData[Rank][RmtTableIndex][Bit + Strobe * MAX_BITS][HIGH] = MrcData->Channel[Channel].RMT_DQData[Rank][RmtTableIndex][Bit ][HIGH];
      } // Bit
    }
  } // strobe
#endif
#if RMT_DEBUG
  MmrcDebugPrint ((MMRC_DBG_DEBUG, "\n"));
  MmrcDebugPrint ((MMRC_DBG_DEBUG, "Worst case: %02d : %02d\n",  MrcData->Channel[Channel].RMT_Data[Rank][RmtTableIndex][LOW], MrcData->Channel[Channel].RMT_Data[Rank][RmtTableIndex][HIGH]));
#endif
  ...
  )
{
  ...
  // Since RMT execute in parallel, we just need to run on the first channel.
  if (Channel != 0) {
    return MMRC_SUCCESS;
  }
  ...
  if ( MrcData->MrcRmtSupport == (RMT_SUPPORT_COMMAND_MODE | RMT_SUPPORT_ENABLED)) {
    Register.Offset = D_CR_SSKPD0_DUNIT_REG;
    SaveScratchpad0 = (UINT32) MemRegRead (DUNIT, Channel, 0, Register);
    Register.Offset = D_CR_SSKPD1_DUNIT_REG;
    SaveScratchpad1 = (UINT32) MemRegRead (DUNIT, Channel, 0, Register);
    while ( 1 )
    {
      MmrcDebugPrint ((MMRC_DBG_MIN, "RMT_CMD\n"));
      Register.Offset = D_CR_SSKPD0_DUNIT_REG;
      MemRegWrite (DUNIT, Channel, 0, Register, 0, 0xF);
      Register.Offset = D_CR_SSKPD1_DUNIT_REG;
      MemRegWrite (DUNIT, Channel, 0, Register, 0, 0xF);

      // Polling until cmd is received
      Register.Offset = D_CR_SSKPD1_DUNIT_REG;
      Scratchpad1 = (UINT32) MemRegRead (DUNIT, Channel, 0, Register);
      while ( Scratchpad1 == 0 )
      {
        Scratchpad1 = (UINT32) MemRegRead (DUNIT, Channel, 0, Register);
      }
      ...
      Register.Offset = D_CR_SSKPD0_DUNIT_REG;
      Scratchpad0 = (UINT32) MemRegRead (DUNIT, Channel, 0, Register);
      ...
      case RMT_COMMAND_EXIT:
        Register.Offset = D_CR_SSKPD0_DUNIT_REG;
        MemRegWrite (DUNIT, Channel, 0, Register, SaveScratchpad0, 0xF);
        Register.Offset = D_CR_SSKPD1_DUNIT_REG;
        MemRegWrite (DUNIT, Channel, 0, Register, SaveScratchpad1, 0xF);
#if RMT_JUMP_POSTCODES
  ...
  )
{
  ...
  for (Channel = ChannelBackup; Channel < MAX_CHANNELS; Channel++) {
    MmrcMemset (MrcData->Channel[Channel].RMT_Data , 0xff, sizeof (UINT16) * (MAX_RANKS + PSEUDO_RANKS) * MAX_RMT_ELEMENTS * 2);
#if RMT_PER_BIT_MARGIN
    MmrcMemset (MrcData->Channel[Channel].RMT_DQData , 0xff, sizeof (UINT16) * (MAX_RANKS + PSEUDO_RANKS) * MAX_RMT_ELEMENTS * MAX_BITS * MAX_STROBES * 2);
#endif
  } // Channel loop ...
  ...
  // find the dram type
  for (Channel = ChannelBackup; Channel < MAX_CHANNELS; Channel++) {
    if (!MrcData->Channel[Channel].ExecuteOnThisChannel || !MrcData->Channel[Channel].Enabled) {
      continue;
    }
    DramType = MrcData->Channel[Channel].DramType;
    break;
  }
  ...
  for (Channel = ChannelBackup; Channel < MAX_CHANNELS; Channel++) {
    ExecuteOnThisChannelOriginal[Channel] = MrcData->Channel[Channel].ExecuteOnThisChannel; // MrcData->Channel[Channel].Enabled)
  }
    ...
    for (PseudoRank = 0; PseudoRank < MAX_RANKS + PSEUDO_RANKS; PseudoRank ++) {
      ...
      while (RmtTablePtr[++i].Signal != MMRC_NA) {
        ...
        if (RmtTablePtr[i].Signal == TxVref) {
          ...
          if((DramType != TypeLpDdr4)) { // && (DramType != TypeLpDdr3)){  // PT temperaroy skip TxVref on LPDDR3
            SkipTxVref = TRUE;
            continue;
          }

          // override the step size of LPDDR4 TxVRef. Because both LPDDR3 and LPDDR4 share the same RmtTable
        ...
        for (Channel = ChannelBackup; Channel < MAX_CHANNELS; Channel++) {
          ...
          if (PseudoRank < MAX_RANKS) {
            ...
            //
            //
            if (!MrcData->Channel[Channel].RankEnabled[PseudoRank]) {
              continue;
            }
        ...
        for (Channel = ChannelBackup; Channel < MAX_CHANNELS; Channel++) {
          ...
          if (RmtTablePtr[i].SkipInitialProgramming == TRUE) {
            ...
            PiGroupCount = MrcData->Channel[Channel].MaxDq[PseudoRank];
            PiMarginGroupCount = MrcData->Channel[Channel].MaxDq[PseudoRank];
        ...
        // temperately override the channel enable to enable even channels
        if ((RmtTablePtr[i].Signal == TxVref) && (DramType == TypeLpDdr3) && (!SkipTxVref)){    // PT added !SkipVref so that the code not execute until late fix which will set the SkipVref to FALSE for LPDDR3
          for (Channel = ChannelBackup; Channel < MAX_CHANNELS; Channel++) {
            if ((Channel % 2) != 0) {
              MrcData->Channel[Channel].ExecuteOnThisChannel = FALSE;
            }
        ...
        // temperately override the channel enable to enable odd channels
        if ((RmtTablePtr[i].Signal == TxVref) && (DramType == TypeLpDdr3) && (!SkipTxVref)){   // PT added !SkipVref so that the code not execute until late fix which will set the SkipVref to FALSE for LPDDR3
          for (Channel = ChannelBackup; Channel < MAX_CHANNELS; Channel++) {
            MrcData->Channel[Channel].ExecuteOnThisChannel = ExecuteOnThisChannelOriginal[Channel];
          } // channel
...
#if RMT_PER_BIT_MARGIN
                MmrcDebugPrint ((MMRC_DBG_MIN, "%4d  %4d     ",MrcData->Channel[Channel].RMT_DQData[PseudoRank][i][Bit + MAX_BITS * Strobe][LOW], MrcData->Channel[Channel].RMT_DQData[PseudoRank][i][Bit + MAX_BITS * Strobe][HIGH]));
#else
                MmrcDebugPrint ((MMRC_DBG_MIN, "%4d  %4d     ", MrcData->Channel[Channel].RMT_Data[PseudoRank][i][LOW],  MrcData->Channel[Channel].RMT_Data[PseudoRank][i][HIGH]));
  ...
  )
{
  ...
  for (ch=0; ch < MAX_CHANNELS; ch++)
  {
    if (MrcData->Channel[ch].Enabled)
    {
      for (rk=0; rk < MAX_RANKS; rk++)
      {
        if (MrcData->Channel[ch].RankEnabled[rk])
        {
  ...
  )
{
  ...
  UINT32 TempValue;
#if INSTRUMENTALGOS
  UINT64 TscInitial, TscFinal, delay, tscpersec;    //kirk
  UINT8   SaveDebugMsgLevel;
  TscInitial = ReadTsc();
  LCTJedecResetCount = 1;
  LCTJedecResetTimeTotal = 0;
  MmrcDebugPrint((MMRC_DBG_MIN, "JedecCount=%llu ms, TimePerJedec=%llu ms\n", LCTJedecResetCount, LCTJedecResetTimeTotal/ LCTJedecResetCount));
  ...
  //if ((MrcData->Channel[Channel].DramType == TypeLpDdr4) && (MrcData->CurrentFrequency == MMRC_2400) ){
 //if ((MrcData->Channel[Channel].DramType == TypeLpDdr4) || (MrcData->Channel[Channel].DramType == TypeLpDdr3)) {
 // if (MrcData->Channel[Channel].DramType == TypeLpDdr4) {
  //  MmrcDebugPrint ((MMRC_DBG_MIN, "*** Skip LP4 LCT ***\n"));
  //  return MMRC_SUCCESS;
  //}

  if (MrcData->Channel[Channel].DramType == TypeLpDdr4){
    for (Loop=0; Loop < 3; Loop++) {
      ControlKnobs[Loop] = ControlKnobsCmdCtl[Loop];
    }
    ...
    for (Loop=0; Loop < 3; Loop++) {
      ControlKnobs[Loop] = ControlKnobsCmdCtl_LPDDR3[Loop];
    }
  }
#endif
#if DDR3_SUPPORT
    ...
    for (Loop=0; Loop < 3; Loop++) {
      ControlKnobs[Loop] = ControlKnobsCmdCtl_DDR3[Loop];
    }
  }
#endif
  //
  ...
  //
  if (!MrcData->RestorePath) {
    MrcData->CpgcOptions.LoopCount = 3;//MrcData->MrcTrainingCpgcExpLoopCnt;
    MrcData->CpgcOptions.NumBursts = 4;//MrcData->MrcCpgcNumBursts;
    MrcData->CpgcOptions.CADB_Harrassment =  TRUE; //Manuel
    for (ch = 0; ch < MAX_CHANNELS; ch++) {
      if (MrcData->Channel[ch].Enabled == TRUE) {
        L_CpgcSetup (MrcData, ch, CPGC_CONFIG_VA /*CPGC_CONFIG_CMD_STRESS*/);
    ...
    for (i=0; i< CMD_NUM_EYEMASKS; i++)
    {
      ...
      LocalEyeMaskCmd[i].MinimumDelayWidth=(EyeMaskPctPiCmdCtl[MrcData->Channel[0].CurrentDdrType][MrcData->CurrentFrequency][i] * GetHalfClk(MrcData, 0))/100;
    ...
    MrcData->CpgcOptions.SetupType = CPGC_CONFIG_VA;
    //
    // Perform the Advanced WriteTraining Algorithm with the Write Control knobs.
    //
    ...
    for (Loop = 0; Loop < MAX_CHANNELS; Loop++) {
      if (MrcData->Channel[Loop].Enabled == TRUE) {
  ...
  //
#ifdef CmdCh0
  DisplayOrRestoreTrainingResults (MrcData, Channel, CmdCh0, TRUE);
  DisplayOrRestoreTrainingResults (MrcData, Channel, CmdCh1, TRUE);
  DisplayOrRestoreTrainingResults (MrcData, Channel, CtlCh0, TRUE);
  DisplayOrRestoreTrainingResults (MrcData, Channel, CtlCh1, TRUE);
#else
  for (ch = 0; ch < MAX_CHANNELS; ch++) {
    if (MrcData->Channel[ch].Enabled == TRUE) {
      DisplayOrRestoreTrainingResults (MrcData, ch, CmdGrp0, TRUE);
      DisplayOrRestoreTrainingResults (MrcData, ch, CtlGrp0, TRUE);
    }
  }
#endif
#if INSTRUMENTALGOS
  ...
  TscFinal = ReadTsc();
  tscpersec = GetTscPerSecond();
  delay = ((TscFinal - TscInitial) * 1000LL) / tscpersec;
  //fakedelay = ((TscFinal - TscInitial) * 1000LL) / 800000000LL;
  //MrcData->MrcDebugMsgLevel = 63;
  MmrcDebugPrint((MMRC_DBG_MIN, "Init=%llu, Final=%llu\nLCT = %llu ms TSCPS=%llu\n", TscInitial, TscFinal, delay, tscpersec));
  delay = (((LCTJedecResetTimeTotal/ LCTJedecResetCount)) * 1000LL) / tscpersec;
  MmrcDebugPrint((MMRC_DBG_MIN, "JedecCount=%llu, TimePerJedec=%llu ms\n", LCTJedecResetCount,  delay));
  //MmrcDebugPrint((MMRC_DBG_MIN, "LCT = %llX ms TSCPS=%llX\n", delay, GetTscPerSecond()));
#endif //INSTRUMENTALGOS
  //
  // End with the exit task for write training.
  //
      ...
      if (MrcData->Channel[Channel].RankEnabled[Rank]) {
        ...
        for (ElementIndex = 0; ElementIndex < NumberOfDelayElementsPerAlgo; ElementIndex++) {
          if (GetSetDdrioGroup2 (MrcData, Channel, Rank, Strobe, TrainingDelay + ElementIndex, CMD_GET_CACHE, &TempValue) == MMRC_SUCCESS) {
            RcvnLinearValue += TempValue * (MrcData->GranularitySteps[MrcData->CurrentBlueprint][ADdll[MrcData->DigitalDll]][MrcData->Channel[Channel].CurrentFrequency][ElementIndex]);
  ...
  )
{
  ...
  for (ElementIndex = 0; ElementIndex < NumberOfDelayElementsPerAlgo; ElementIndex++) {
    MmrcDebugPrint ((MMRC_DBG_MIN, "%3d ",
      MrcData->GranularitySteps[MrcData->CurrentBlueprint][ADdll[MrcData->DigitalDll]][MrcData->Channel[Channel].CurrentFrequency][ElementIndex]
      ));
  }
        ...
        for (Strobe = 0; Strobe < MAX_STROBES; Strobe++) {
          ...
          for (ElementIndex = 0; ElementIndex < NumberOfDelayElementsPerAlgo; ElementIndex++) {
            ...
            if (GetSetDdrioGroup2 (MrcData, Channel, Rank, Strobe, TrainingDelay + ElementIndex, AccessMethod, &TempValue) == MMRC_SUCCESS) {
              RecalculatedDelay += TempValue * (MrcData->GranularitySteps[MrcData->CurrentBlueprint][ADdll[MrcData->DigitalDll]][MrcData->Channel[Channel].CurrentFrequency][ElementIndex]);
              //
  ...
  )
{
  ...
  CurrentPlatform       = MrcData->Channel[Channel].CurrentPlatform;
  CurrentFrequency      = MrcData->Channel[Channel].CurrentFrequency;
  CurrentConfiguration  = MrcData->Channel[Channel].CurrentConfiguration;
  CurrentType           = MrcData->Channel[Channel].CurrentDdrType;
  ...
  )
{
  ...
  for (Channel; Channel < MAX_CHANNELS; Channel++) {
    ...
    if (Dim2ConsolidateStrobes == 1) {
      ...
      StrobeLoopCount = MrcData->Channel[Channel].MaxDq[Rank];
    }
  ...
  )
{
...
#if RTWT_SIM == 1
  {
    ...
    if (Dim1Index == TxVref) {
      ...
      for (Knob=0; Knob < MAX_KNOBS; Knob++) {
        for (Strobe=0; Strobe < MAX_STROBES; Strobe++) {
          for (Bit=0; Bit < MAX_BITS; Bit++) {
            GetSetDdrioGroup (MrcData, 0, Channel, 0, Rank, Strobe, Bit, MrcData->DVFSFreqIndex, 0xFF, TxDqDelay, CMD_GET_REG, &Pi);
            GetSetDdrioGroup (MrcData, 0, Channel, 0, Rank, Strobe, Bit, MrcData->DVFSFreqIndex, 0xFF, TxDqBitDelay, CMD_GET_REG, &PiBit);
            Pi -= PiBit;
            State=0;
    ...
    if (Dim1Index == RxVref) {
      ...
      for (Knob=0; Knob < MAX_KNOBS; Knob++) {
        for (Strobe=0; Strobe < MAX_STROBES; Strobe++) {
          for (Bit=0; Bit < MAX_BITS; Bit++) {
            GetSetDdrioGroup (MrcData, 0, Channel, 0, Rank, Strobe, Bit, MrcData->DVFSFreqIndex, 0xFF, Ctle, CMD_GET_REG, &CTLE);
            GetSetDdrioGroup (MrcData, 0, Channel, 0, Rank, Strobe, Bit, MrcData->DVFSFreqIndex, 0xFF, RxDqsPDelay, CMD_GET_REG, &Pi);
            GetSetDdrioGroup (MrcData, 0, Channel, 0, Rank, Strobe, Bit, MrcData->DVFSFreqIndex, 0xFF, RxDqBitDelay, CMD_GET_REG, &PiBit);
            Pi -= PiBit;
            State=0;
            for (Vref=(UINT8) Dim1Min[Channel][Strobe]; Vref < (UINT8) Dim1Max[Channel][Strobe]; Vref+=(UINT8) Dim1Step) {
              //
     ...
     for (Knob=0; Knob < MAX_KNOBS; Knob++) {
      for (Strobe=0; Strobe < MAX_STROBES; Strobe++) {
        for (Bit=0; Bit < MAX_BITS; Bit++) {
         GetSetDdrioGroup (MrcData, 0, Channel, 0, Rank, Strobe, Bit, MrcData->DVFSFreqIndex, 0xFF, Ctle, CMD_GET_REG, &CTLE);
         GetSetDdrioGroup (MrcData, 0, Channel, 0, Rank, Strobe, Bit, MrcData->DVFSFreqIndex, 0xFF, RxVref, CMD_GET_REG, &VREF);
  ...
  for (Channel = ChannelBackup; Channel < MAX_CHANNELS; Channel++) {
    ...
    for (Knob = 0; Knob < Dim1Knobs; Knob++) {
      for (Element = 0; Element < NumberElements; Element++) {
        ...
        //MmrcDebugPrint ((MMRC_DBG_MIN, "Start H:%04d L:%04d Lim H:%04d L:%04d\n", Dim1Start[Channel][Knob][Element][HIGH], Dim1Start[Channel][Knob][Element][LOW], Dim1Max[Channel][Element], Dim1Min[Channel][Element]));
        for (Bits = 0; Bits < MaximumBits; Bits++) {
          ...
          //MmrcDebugPrint ((MMRC_DBG_MIN, "C%01dK%01dE%01db%01bLH:%d\n", Channel, Knob, Element, Bits, LowHighFlag[Channel][Knob][Element][Bits]));
         }
        //
  ...
  while (!TestFinished) {
        ...
        for (Element = 0; Element < NumberElements; Element++) {
          ...
          if (!ElementBitDone) {
            for (Bits = 0; Bits < 1/*MaximumBits*/; Bits++) {
              // Peter  added another case that we need to set the offset.
    ...
    for (Channel = ChannelBackup; Channel < MAX_CHANNELS; Channel++) {
      ...
      MmrcDebugPrint ((MMRC_DBG_MIN, "CH%02dR%02d", Channel, Rank));
      //
      ...
      for (Knob = 0; Knob < Dim1Knobs; Knob++) {
        for (Element = 0; Element < NumberElements; Element++) {
          ...
          GetSetDdrioGroup2 (MrcData, Channel, Rank, Element, Dim1Index, CMD_GET_REG, &TempVal);
          //MmrcDebugPrint ((MMRC_DBG_MIN, "|%04d|", TempVal));
        }
      }
      MmrcDebugPrint((MMRC_DBG_MIN, "\n"));
      for (Knob = 0; Knob < Dim1Knobs; Knob++) {
        for (Element = 0; Element < NumberElements; Element++) {
          ...
          for (Bits = 0; Bits < MaximumBits; Bits++) {
            ...
            if (Phase[Channel][Knob][Element][Bits] == 4) {
              ...
              for (st = 0; st < NumberElements; st++) {
                for (Bits=0; Bits < MaximumBits; Bits++) {
                  LowHighFlag[Channel][Knob][st][Bits]   = HIGH;
                  //MmrcDebugPrint ((MMRC_DBG_MIN, "C%01dK%01dE%01db%01dLH:%d\n", Channel, Knob, st, Bits, LowHighFlag[Channel][Knob][Element][Bits]));
                 ...
                 // Value[Channel][Knob][Element] = Results[Channel][Knob][Element][Bits][HIGH] - Results[Channel][Knob][Element][Bits][LOW];

                  //
                  // Program HIGH side initial values. If user opts to skip this step, the HIGH side sweep
                  // will start where the LOW side ended.
                  //
                  Phase[Channel][Knob][st][Bits] = 0;
                  TestFinished = FALSE;
  ...
  for (LowHighFlag = LOW; LowHighFlag <= HIGH; LowHighFlag++) {
    ...
    do {
      ...
      for (Channel = ChannelBackup; Channel < MAX_CHANNELS; Channel++) {
        ...
        MmrcDebugPrint ((MMRC_DBG_MIN, "CH%02dR%02d %s %03d\n", Channel, Rank,
      ...
      // Create the 1D Sweep
      //
      Create1DSweepLastPass (MrcData, Channel, Rank, Dim1Index, Dim1Knobs, Dim1StartPoint, Dim1Min, Dim1Max, Dim1Step, FALSE, FALSE, FALSE, MrcData->Channel[Channel].MaxDq[Rank], ExecuteTest, Results[DimYOffset].Sample, HeaderLabel, 1);
      //
  ...
  )
{
  ...
  case MrcMemInit:
    for (Channel = 0; Channel < MAX_CHANNELS; Channel++) {
      MrcData->DynamicVars[Channel][ENABLE_RUN_TEST] = (UINTX) (MrcData->Channel[Channel].Enabled & MrcData->Channel[Channel].ExecuteOnThisChannel);
      //
  ...
  )
{
  ...
  DeadLoopMrcData->MrcDebugMsgLevel = MMRC_DBG_VERBOSE;
  //
  // First display all delays which is useful to see where training went wrong.
  //
#if PHYVIEW_TABLE
  for (Channel = 0; Channel < MAX_CHANNELS; Channel++) {
    if (DeadLoopMrcData->Channel[Channel].Enabled) {
  ...
  )
{
  ...
  if (MrcData->Channel[Channel].DramType == TypeLpDdr3) {
    //
    // Drive CKE high using DRMC register
    //
    MrcData->DynamicVars[Channel][REG_DATA] = (Rank==0?2:1);//3;
    MmrcExecuteTask(MrcData, DRIVE_CKE, NO_PRINT, Channel);
    //
    // Restore the system back to the non-ca training status.
    //
    MrcData->DynamicVars[Channel][CAVREF] = 0x1f;
    MmrcExecuteTask(MrcData, EXIT_CA_TRAINING, NO_PRINT, Channel);
    //
    // Issue the MR48 command.
    //
    MmrcWriteMRSCommand(MrcData, Channel, Rank, 1, FALSE, MRW48_LP3);
    MySimStall(1000);
    MrcDelay (MrcData, MICRO_DEL, 2);
    //
    // Reenter CA Training.
    //
    MrcData->DynamicVars[Channel][CAVREF] = 0x1f;
    ...
    MmrcExecuteTask(MrcData, DRIVE_CKE, NO_PRINT, Channel);
    //
    // Loop through the patterns and retest the same patterns comparing with the expected results.
    //
    for (PatternIndex = 0; PatternIndex < NUMBER_EARLY_COMMAND_PATTERNS; PatternIndex++) {
      MmrcWriteMRSCommand(MrcData, Channel, Rank, 1, FALSE, (EarlyCommandPatternsRise[PatternIndex] | (EarlyCommandPatternsFall[PatternIndex] << 10)));
      //CA[9, 4] tCK rise to the Upper DRAM => DQCALRDAT[24, 16]
      //CA[9, 4] tCK fall  to the Upper DRAM => DQCALRDAT[25, 17]
#ifdef SIM
      ExpectedResult = EarlyCommandResults48[PatternIndex];
#else
      DataRise = (( (EarlyCommandPatternsRise[PatternIndex]& BIT9) >> 9) << 24) | (((EarlyCommandPatternsRise[PatternIndex] & BIT4) >> 4) << 16);
      DataFall = (( (EarlyCommandPatternsFall[PatternIndex]& BIT9) >> 9) << 25) | (((EarlyCommandPatternsFall[PatternIndex] & BIT4) >> 4) << 17);
      //ExpectedResult = DataRise;
      //ExpectedResult |= ((DataFall & BIT1) << 9) | ((DataFall & BIT0) << 8);
      ExpectedResult = DataRise | DataFall;
      //ExpectedResult =  DeSwizzle(MrcData, Channel, ExpectedResult);
      //ExpectedResult = EarlyCommandResults48[Channel][PatternIndex];
#endif
      MySimStall(500);
      MrcDelay (MrcData, MICRO_DEL, 2);
      //
      ...
      // Swizzle the result reading back. Only 16 bits are valid to compare.
      SwizzleResult = Swizzle (MrcData, Channel, (UINT32)MrcData->DynamicVars[Channel][REG_DATA])  & (BIT16|BIT17|BIT24|BIT25);
    ...
    // Drive CKE high using DRMC register
    //
    //MrcData->DynamicVars[Channel][REG_DATA] = 3;
    MrcData->DynamicVars[Channel][REG_DATA] = (Rank==0?2:1);
    MmrcExecuteTask(MrcData, DRIVE_CKE, NO_PRINT, Channel);
    //
    // Restore the system back to the non-ca training status.
    ...
    MmrcExecuteTask(MrcData, EXIT_CA_TRAINING, NO_PRINT, Channel);
    //
    // Issue the MR41 command.
    //
    MmrcWriteMRSCommand(MrcData, Channel, Rank, 1, FALSE, MRW42_LP3);
    MySimStall(1000);
    MrcDelay (MrcData, MICRO_DEL, 2);
    MmrcWriteMRSCommand(MrcData, Channel, Rank, 1, FALSE, MRW41_LP3);
    MySimStall(1000);
    MrcDelay (MrcData, MICRO_DEL, 2);
    //
    // Reenter CA Training.
...
)
{
  ...
  CCCAdjustment = MinCCC / GetHalfClk(MrcData, Channel);
  CCCAdjustment -= 2;
  CCCAdjustment = CCCAdjustment * GetHalfClk(MrcData, Channel);

  for (CccLoop = 0; CccLoop < MAX_CMDS; CccLoop++) {
    CmdTempValue[CccLoop] -= CCCAdjustment;
    CtlTempValue[CccLoop] -= CCCAdjustment;
    CkTempValue[CccLoop]  -= CCCAdjustment;
  }
  ...
  for (Rank = 0; Rank < MAX_RANKS; Rank++)
  {
    //Make sure all ranks are updated so that the cache has the right values
    for (CccLoop = 0; CccLoop < MAX_CMDS; CccLoop++) {
    ...
    MinCCC +=1;
    GetSetDdrioGroup2 (MrcData, Channel, Rank, 0, CkeAll, CMD_SET_VAL_FC_UC, &MinCCC);
  }
...
)
{
  ...
  for (CccLoop = 0; CccLoop < MAX_CMDS; CccLoop++) {
    ...
    if (CommandTemp[CccLoop] < MinCCC)
      MinCCC = CommandTemp[CccLoop];
    ...
    if (ControlTemp[CccLoop] < MinCCC)
      MinCCC = ControlTemp[CccLoop];
    ...
    if (ClockTemp[CccLoop] < MinCCC)
      MinCCC = ClockTemp[CccLoop];
  } // for CccLoop
...
////TODO: Clean up this construct. we shouldn't need the If(0) part
#ifdef BXTPMRC
  if (MrcData->Channel[Channel].DramType == TypeLpDdr4) {
    if (Channel==0) {
      MmrcDebugPrint ((MMRC_DBG_MIN, "*** Skip LP4 ECT ***\n  "));
    }
    return MMRC_SUCCESS;
#else
 if (0) {
  ...
  for (Rank = 0; Rank < MAX_RANKS; Rank++) {
    ...
    //
    MrcData->DynamicVars[Channel][CAVREF] = DeSwizzle(MrcData, Channel, ECT_LP4_VREF);
    ...
    if (MrcData->Channel[Channel].DramType == TypeLpDdr4) {
      ...
      //MmrcExecuteTask(MrcData, CPGC_RUN_TEST, NO_PRINT, Channel);
      L_CpgcRunTest (MrcData, Channel, (UINT16*) NULL, (UINT32*) NULL, (UINT32*) NULL, (UINT32*) NULL);
      MySimStall(500);
      MrcDelay (MrcData, NANO_DEL, 100);
      for (Strobe=0; Strobe < MAX_STROBES; Strobe++) {
        if (!IsStrobeValid (MrcData, Channel, Rank, Strobe)) {
  ...
  )
{
  ...
  for (i = 0; i < MAX_IO_LEVELS; i++) {
    if ((((1 << MrcData->Channel[Channel].CurrentPlatform)      & PfctAndIoLevelToBlueprint[i].PMask) != 0) &&
        (((1 << MrcData->Channel[Channel].CurrentFrequency)     & PfctAndIoLevelToBlueprint[i].FMask) != 0) &&
        (((1 << MrcData->Channel[Channel].CurrentConfiguration) & PfctAndIoLevelToBlueprint[i].CMask) != 0) &&
        (((1 << MrcData->Channel[Channel].CurrentDdrType)       & PfctAndIoLevelToBlueprint[i].TMask) != 0) &&
...
{
#if ENABLE_RUN_TEST
  MrcData->DynamicVars[Channel][ENABLE_RUN_TEST] = (UINTX) (MrcData->Channel[Channel].RankEnabled[Rank] &
                                                            MrcData->Channel[Channel].ExecuteOnThisChannel &
                                                            MrcData->Channel[Channel].Enabled);
  if (MrcData->DynamicVars[Channel][ENABLE_RUN_TEST] == 0) {
#else
  UINT32 X;
  X = (MrcData->Channel[Channel].RankEnabled[Rank] &
                                                            MrcData->Channel[Channel].ExecuteOnThisChannel &
                                                            MrcData->Channel[Channel].Enabled);
  if (X == 0) {
#endif
...
#if TRAINING_ALGOS == 1 && LDO_TRAINING == 1
...
{
  LDOTraining_Base (MrcData, CapsuleStartIndex,  StringIndex,   Channel,  LDO_CAPASS);
}
...
{
  LDOTraining_Base (MrcData, CapsuleStartIndex,  StringIndex,   Channel,  LDO_DATAPASS);
}
...
{
  LDOTraining_Base (MrcData, CapsuleStartIndex,  StringIndex,   Channel,  LDO_BOTH);
}
...
)
{
  UINT32 TargetV;
  UINT32 VccFARmV = 850;
  UINT32 VmemmV = 1200/*TODO*/; //From Bios
  UINT32 PanicVHi, PanicVLo;
  UINT32 PanicHImV=62; // PanicHImV defaults to 62mV.  Max value of 62 mV
  UINT32 PanicLOmV=62; // PanicLOmV defaults to 62mV.  Max value of 62 mV
  INT32  TrainOrder[4][LDO_COEFFS] = {{0,1,2,5,4,3,-1}, {0,2,3,4,1,6,5}, {0,1,2,3,4,5,-1}, {0,1,2,4,5,3,-1}};
  UINT32 DataA[4][LDO_COEFFS] = {{0xDB556DAA, 0x82084224, 0xC1860C60, 0x1C71C71C, 0xE7C7C3E0, 0xFDF9FBF0, 0x0},
                        {0xAAAAAAAA, 0x82110420, 0xDB6DB6DA, 0xC1860C60, 0xEE71DCE0, 0xF0F079E0, 0xFDF87E0E},
                        {0xAAAAAAAA, 0x91082420, 0xDB6DB6DA, 0xCC618330, 0xEE3838E0, 0xFDF87E0E, 0x0},
                        {0xDB556DAA, 0x910C1304, 0xC618C318, 0xE739CE72, 0x1C71C71C, 0xFDF87E0E, 0x0}};
  UINT32 DataB[4][LDO_COEFFS] = {{0xB6AADB55, 0x24421041, 0x0C306306, 0xE38E38E0, 0x7C7C3E0E, 0x0FDF9FBF, 0x0},
                        {0x55555555, 0x08211042, 0x6DB6DB6D, 0x0C306306, 0x738EE707, 0x0F0F079E, 0xBF0FC1DF},
                        {0x55555555, 0x09108242, 0x6DB6DB6D, 0x331860CC, 0x1DC7071C, 0xBF0FC1DF, 0x0},
                        {0xB6AADB55, 0x89826090, 0x30C618C6, 0x39CE7397, 0xE38E38E0, 0xBF0FC1DF, 0x0}};

  UINT16 CmdA[4][LDO_COEFFS] = {{0xD5AA, 0x9044, 0xC618, 0xFCFC, 0xF1F0, 0xE0E0, 0x0},
                                {0xAAAA, 0xEE70, 0x8220, 0xDADA, 0xC618, 0xFDF8, 0xF0F0},
                                {0xAAAA, 0x9104, 0xDADA, 0xCC60, 0xEE0E, 0xFC1E, 0x0},
                                {0xD5AA, 0x9060, 0xC618, 0xFDF0, 0xB972, 0xE370, 0x0}};
  UINT16 CmdB[4][LDO_COEFFS] = {{0xAB55, 0x4112, 0x30C6, 0xCFCF, 0x1F0F, 0x1C1C, 0x0},
                                {0x5555, 0x7387, 0x1104, 0xB5B5, 0x30C6, 0xEFC7, 0x0F0F},
                                {0x5555, 0x4412, 0xB5B5, 0x3183, 0x7077, 0xF7E0, 0x0},
                                {0xAB55, 0x4182, 0x1863, 0x0FDF, 0xAE5C, 0x1B87, 0x0}};

  UINT32  DqBase = 0;
  UINT32  Scale, BGACh;
  UINTX   Value;
  REGISTER_ACCESS Register;
  UINT32  MaxValue, Ieff, CoeffValue, MaxAtLowerScale, NumCoeffs[2];
  BOOLEAN POP  = 1; //TODO
  UINT32  Freq = 3200; //TODO
  BOOLEAN BGA  = 1; //TODO
  UINT32  TcodeCA, TcodeCLK, TcodeDQ;
  UINT32  FBDiffLimit, CoeffDqOdt, IndexDqOdt, FFValue[2], FFBitMask, ScaleBack;
  UINT32  FScale[2], FCh[2], OldScale, SumDiff, Coeff, NewCoeff;
  BOOLEAN Done=0;
  UINT32  ScaledTcodeCLK, ScaledTcodeCA, ScaledTcodeDQ, ScaledTcodeCADQ, Tcode;
  int     i,j;
  int     PassStart, PassEnd;
  UINT32  IpanicuA, Cmim = 3200, deltaV=8, Freq_dclk=1600/*TODO*/; //TODO
  UINT32  IffleguA, PMOSPanic, InmosuA, VccMEMmV = 1200 /*TODO*/, Rcprcomp = 80/*TODO*/, NMOSPanic, Ron_cpu = 20 /*made up*/ /*TODO*/, Rodt_dram = 80/*TODO*/, Rstray=2, UpdatePanicComp = (Rodt_dram != 0)/*TODO*/;
  UINTX   scram_save;
  D_CR_SCRAMCTRL_DUNIT_STRUCT * scramreg;
  ...
  if (SelectedTrainings == LDO_BOTH) //Run both one at a time
  {
    PassStart = LDO_CAPASS;
    PassEnd   = LDO_DATAPASS;
  }
  else if (SelectedTrainings == LDO_DATAPASS) //Run Only Data
  {
    PassStart = LDO_DATAPASS;
    PassEnd   = LDO_DATAPASS;
  }
  else //CA Pass Only
  {
  ...
  if (MrcData->Channel[Channel].DramType == TypeLpDdr4) {
    ...
    if   ((Freq>1600) && (POP))     BGACh = 0;    // Short channel at 3200
    else if ((Freq>1600) && (BGA))  BGACh = 1;    // Long channel at 3200
    else                            BGACh = 2;    // 1600 MTs for any length channel
                                                  // BGACh = 3 supported in HW but not currently used

    TargetV = ((VccFARmV*96)/VmemmV) - 64;
    PanicVHi  = (PanicHImV*384) / (VmemmV*3); //Could change during PostSi
    PanicVLo  = (PanicLOmV*384) / (VmemmV*3); //Could change during PostSi
    MrcData->DynamicVars[Channel][BGACH]    = BGACh;
    MrcData->DynamicVars[Channel][TARGETV]  = TargetV;  //TODO
    MrcData->DynamicVars[Channel][PANICVHI] = PanicVHi; //TODO
    MrcData->DynamicVars[Channel][PANICVLO] = PanicVLo; //TODO
    MmrcDebugPrint ((MMRC_DBG_MED, "Init\r\n"));
    MmrcExecuteTask(MrcData, LDO_TRAIN_INIT, 0x56, Channel);
    MmrcDebugPrint ((MMRC_DBG_MED, "CADB Setup\r\n"));
    MmrcExecuteTask (MrcData, CPGC_CADB1, 0x9B, Channel); //Init CADB

    for (i=PassStart; i<= PassEnd; i++)
    {
      if (i == LDO_CAPASS)
      {
        DqBase = 0;
      ...
      while (Scale > 0)
      {
        MmrcDebugPrint ((MMRC_DBG_MED, "\r\nSelecting Scale %d: ", Scale));
        MrcData->DynamicVars[Channel][LDOSCALE] = Scale;
        if (i == LDO_CAPASS) MmrcExecuteTask(MrcData, SET_DQ_SCALE, 0x57, Channel);
        else MmrcExecuteTask(MrcData, SET_DQ_SCALE, 0x58, Channel);

        MaxValue=0;
        for (j=0; j < LDO_COEFFS; j++)
        {
          Ieff = TrainOrder[BGACh][j];
          if (Ieff == -1) continue;
          NumCoeffs[i]++;
          if (i == LDO_CAPASS)
          {
            //Setup CADB Pattern
            MrcData->DynamicVars[Channel][LDO_CADB_EVEN_PATTERN] = CmdA[BGACh][Ieff];
            MrcData->DynamicVars[Channel][LDO_CADB_ODD_PATTERN] = CmdB[BGACh][Ieff];
            MmrcExecuteTask (MrcData, CPGC_LDO_CADB, 0xA4, Channel);
            //Enabled CA Harrasment
            MmrcExecuteTask (MrcData, CPGC_DESELECT_EN, 0x9D, Channel);
          }
          else
          {
            //Setup Data Pattern
            MrcData->DynamicVars[Channel][LDO_DPAT_EVEN_PATTERN] = DataA[BGACh][Ieff];
            MrcData->DynamicVars[Channel][LDO_DPAT_ODD_PATTERN] = DataB[BGACh][Ieff];
            MmrcExecuteTask (MrcData, CPGC_LDO_DPAT, 0xA5, Channel);
            //Setup Long Loop Mode
            MrcData->DynamicVars[Channel][NUM_BURSTS_EXP] = 100000;
            MrcData->DynamicVars[Channel][NUM_LOOPS_EXP] = 100000;
            MmrcExecuteTask (MrcData, CPGC_LOOPS, 0x8C, Channel);
          }
          MmrcExecuteTask (MrcData, CPGC_RUN_TEST, 0x85, Channel);

          MrcData->DynamicVars[Channel][LDOCOEF] = DqBase + Ieff + 1;
          MmrcExecuteTask(MrcData, LDO_TRAIN_LOOP, 0x59, Channel);
          if (i == LDO_CAPASS)
          {
            MmrcExecuteTask (MrcData, CPGC_DESELECT_DIS, 0x9C, Channel);
          }
          else
          {
            MmrcExecuteTask (MrcData, CPGC_STOP_TEST, 0x8F, Channel);
          }

          // Find Coefficient with the maximum value
          if (i == LDO_CAPASS) Register.Offset = TXFFCA0_DVFS_LDO_REG;
          else Register.Offset = TXFFDQ0_DVFS_LDO_REG;
          Value = (UINT32) MemRegRead (LDO, Channel, 0, Register);
          CoeffValue = ((Value & 0x7FFFFFF) >> (4*Ieff)) & 0xF;

          if (MaxValue < CoeffValue) MaxValue = CoeffValue;
          MmrcDebugPrint ((MMRC_DBG_MED, "\r\nCoeff#%d MAX=%d Coeff=%d", Ieff, MaxValue, CoeffValue));
        }
        // Cannot go any lower in scale such that we are done at this point
        if (Scale == 1) break;

        // predict what the MaxCoeff will do if we go to a higher scale value
        MaxAtLowerScale = (MaxValue*Scale + (Scale-1)/2) / (Scale-1);
        if (MaxAtLowerScale > 15)  break;          // Stop here
        else                       Scale -= 1;    // Keep reducing Scale

        if (i == LDO_CAPASS)
        {
          //Setup CADB Pattern
          MrcData->DynamicVars[Channel][LDO_CADB_LFSR0] = CmdA[BGACh][Ieff];
          MrcData->DynamicVars[Channel][LDO_CADB_LFSR1] = CmdA[BGACh][Ieff];
          MrcData->DynamicVars[Channel][LDO_CADB_LFSR2] = CmdA[BGACh][Ieff];
          MmrcExecuteTask (MrcData, CPGC_LDO_CADB_LFSR, 0x9D, Channel);
        ...
        {
          //Setup Data Pattern
          MrcData->DynamicVars[Channel][LDO_DPAT_LFSR_SEED] = DataA[BGACh][Ieff];
          MmrcExecuteTask (MrcData, CPGC_LDO_DPAT_LFSR, 0xA7, Channel);
          //Setup Long Loop Mode
          MrcData->DynamicVars[Channel][NUM_BURSTS_EXP] = 100000;
          MrcData->DynamicVars[Channel][NUM_LOOPS_EXP] = 100000;
          MmrcExecuteTask (MrcData, CPGC_LOOPS, 0x8C, Channel);
        ...
        //Enable Scrambler for DQ
        Register.Offset = D_CR_SCRAMCTRL_DUNIT_REG;
        Register.ShiftBit = 0;
        Register.Mask = 0xFFFFFFFF;
        Value = MemRegRead (DUNIT, Channel, 0, Register);
        scram_save = Value;
        scramreg = (D_CR_SCRAMCTRL_DUNIT_STRUCT*) &Value;
        scramreg->Bits.key = 0xBEEF;
        scramreg->Bits.scrm_en = 1;
        scramreg->Bits.clockgate = 0;
        MemRegWrite(DUNIT, Channel, 0, Register, *((UINTX *) scramreg), 0xF);
        ...
        for (j=0; j < LDO_COEFFS; j++)
        {
          ...
          //Stop Test
          MmrcExecuteTask (MrcData, CPGC_STOP_TEST, 0x8F, Channel);
          if (i == LDO_CAPASS) MmrcExecuteTask (MrcData, CPGC_DESELECT_DIS, 0x9C, Channel);
        }
      }
    }
    TcodeCA=TcodeCLK=TcodeDQ=0;
    FBDiffLimit = (LDO_COEFFS*64) / (2*(8+2+1) + 6 + 2 + 2);
    CoeffDqOdt=0;
    IndexDqOdt=(TrainOrder[BGACh][6] == -1)? TrainOrder[BGACh][5] : TrainOrder[BGACh][6];
    FScale[0]=FScale[1]=0;
    FFValue[0]=FFValue[1]=0;
    FCh[0]=FCh[1]=0;

    FFBitMask=0x7FFFFFF; // FF Coeffs only uses 26:0
    ScaleBack = 80;      // Use 80% of the original value
    for (i=PassStart; i<= PassEnd; i++) //Bus
    {
      if (i == LDO_CAPASS)
      {
        Register.Offset = TXFFCA0_DVFS_LDO_REG;
        MmrcDebugPrint ((MMRC_DBG_MED, "\r\nFine Training CA"));
      }
      else
      {
        Register.Offset = TXFFDQ0_DVFS_LDO_REG;
        MmrcDebugPrint ((MMRC_DBG_MED, "\r\nFine Training DQ"));
      }

      // Read FF codes and split up fields
      Value = (UINT32) MemRegRead (LDO, Channel, 0, Register);
      FScale[i] = (Value>>27) & 0x7;  // Scale bits
      FCh[i] = (Value>>30) & 0x3;     // CH matric bits
      OldScale = FScale[i];
      Done = 0;
      while (Done==0) // Increment Scale until FB does not overflow
      {
        SumDiff = 0;
        FFValue[i] = 0; // Keep non-Coeff bits (Scale & CH)
        for (j=0; j<LDO_COEFFS; j++) //Index
        {
          Coeff = (Value >> (j*4)) & 0xF; // Extract 4 bit Coeff
          NewCoeff = ((FScale[i]*ScaleBack*Coeff) + (ScaleBack/2) + (FScale[i]))/(100*OldScale);
          SumDiff += (((Coeff*OldScale) - (NewCoeff*FScale[i]))/FScale[i]);
          FFValue[i] |= (NewCoeff << (i*4));

          if ((i == LDO_CAPASS) && (j==0)) TcodeCLK=2*SumDiff;
          if ((i == LDO_DATAPASS) && (j==IndexDqOdt)) CoeffDqOdt = Coeff*OldScale;
        }
        MmrcDebugPrint ((MMRC_DBG_MED, "\r\nSumDiff:%d FBDiffLimit:%d", SumDiff, FBDiffLimit));
        //Limit FB within the threshhold by incrementing scale
        if (SumDiff > FBDiffLimit)  FScale[i] += 1;
        else                        Done = 1;
      }
      FFValue[i] |= (FCh[i]<<30) | (FScale[i]<<27);
      if (i == LDO_CAPASS) TcodeCA = (8*SumDiff) / NumCoeffs[i];      // 6 CA + 2 CS lanes
      else             TcodeDQ = (2*11*SumDiff) / NumCoeffs[i];      // 2 Byte*(8DQ+2DQS+1DM)
    }
    // Update Tcode Register
    // CA Coeff are multiplied by ScaleCA in HW but FB Codes are multipled by ScaleDQ in HW
    // CLK is toggling in all modes (Idle, CA, DQ, DQ+CA)
    ScaledTcodeCLK  = ((TcodeCLK*FScale[1]) + (FScale[0]/2))/FScale[0];
    ScaledTcodeCLK  = (ScaledTcodeCLK > 63) ? 63 : ScaledTcodeCLK;
    ScaledTcodeCA   = (((TcodeCA*FScale[1]) + (FScale[0]/2))/FScale[0]) + TcodeCLK;
    ScaledTcodeCA   = (ScaledTcodeCA > 63) ? 63 : ScaledTcodeCA;
    ScaledTcodeDQ   = TcodeDQ + ScaledTcodeCLK;
    ScaledTcodeDQ   = (ScaledTcodeDQ > 63) ? 63 : ScaledTcodeDQ;
    ScaledTcodeCADQ = ScaledTcodeDQ + ScaledTcodeCA - ScaledTcodeCLK;
    ScaledTcodeCADQ = (ScaledTcodeCADQ > 63) ? 63 : ScaledTcodeCADQ;
    Tcode = (2 <<30) | (ScaledTcodeCLK<<18)| (ScaledTcodeCA<<12) | (ScaledTcodeDQ<<6) | (ScaledTcodeCADQ);

    Register.Offset = TXTCODE0_DVFS_LDO_REG;
    MemRegWrite (LDO, Channel, 0, Register, Tcode, 0xF);

    // Panic Comp
    // Only update PanicComp when ODT enable since we need to be able to calculate a current
    // POR is to always enabled ODT at the beginning of boot and have power training turn it off later
    // As a result, we can use the ODT training results to calculate the PMOS comp limits
    //
    // Target Current for Panic is calculated to move VccFAR by 15mV in 1 cycle
    // PMOS PanicComp: Use Coeff5 (including ScaleDQ) to calculate current per leg based on Ron/Rodt
    // NMOS PanicComp: Hardware Rcomp loop will provide NumNmosLeg to produce (Vmem-VccFar)/80 ~ 3.4mA
    //                 BIOS needs to provide a scale factor to reach the target current

    IpanicuA = Cmim*deltaV*Freq_dclk/1000;    // Cmin=3200pF, deltaV=8mV, Freq_dclk=800-1600 MHz
    Register.Offset = TXPANICCOMP_LDO_REG;
    Value = (UINT32) MemRegRead (LDO, Channel, 0, Register);

    if (UpdatePanicComp)        // Only update PanicComp when ODT is enable
    {
      IffleguA = (1000*VccFARmV) / (Ron_cpu+Rodt_dram+Rstray) / CoeffDqOdt;  // Rstray = 2 ohms
      PMOSPanic = IpanicuA / IffleguA;
      PMOSPanic = (PMOSPanic > 63) ? 63 : 0;
      Value = ((Value & 0xFFFFF0FF) | (PMOSPanic << 8));
    }

    InmosuA = (1000 * (VccMEMmV - VccFARmV)) / Rcprcomp;          // Rcprcomp = 80ohm
    NMOSPanic = IpanicuA / InmosuA;
    NMOSPanic = (NMOSPanic > 15) ? 15 : 0;
    Value = ((Value & 0xFFFFFFF0) | (NMOSPanic << 0));//BitSwap(CRValue, NMOSPanic, BitStart=(DVFS ? 0 : 4), 4);
    Register.Offset = TXPANICCOMP_LDO_REG;
    MemRegWrite (LDO, Channel, 0, Register, Value, 0xF);

    // Enable Panic Drive before turning on feedback or changing the FF values
    //        ForcePanic  CBit      FBPwrDown FBPwrDown   PanicSR  PanicClk  PanicCA PanicDQ
    Value  = (0<<27) | (0<<15) | (1<<14) | (1<<13) | (1<<12) | (3<<10) | (1<<8) | (1<<6);
    Value |= (PanicVHi<<3) | (PanicVLo<<0);
    Register.Offset = TXPANIC_LDO_REG;
    MemRegWrite (LDO, Channel, 0, Register, Value, 0xF);

    // Update FB Register to enable the feedback register
    //           TargetV      FastBW=4  SlowBW=15  Ripple=1 EnReg    EnFB     EnAll Modes
    Value = (TargetV<<13) | (3<<10) | (3<<7)   | (1<<4) | (1<<3) | (1<<2) | (0<<0) ;
    Register.Offset = TXFB_LDO_REG;
    MemRegWrite (LDO, Channel, 0, Register, Value, 0xF);

    // Update FF Register
    if ((PassStart == LDO_DATAPASS) || (PassEnd == LDO_DATAPASS))
    {
      Register.Offset = TXFFDQ0_DVFS_LDO_REG;
      MemRegWrite (LDO, Channel, 0, Register, FFValue[LDO_DATAPASS], 0xF);
    }
    if ((PassStart == LDO_CAPASS) || (PassEnd == LDO_CAPASS))
    {
      Register.Offset = TXFFCA0_DVFS_LDO_REG;
      MemRegWrite (LDO, Channel, 0, Register, FFValue[LDO_CAPASS], 0xF);
    }
    MemRegWrite(DQCCC, Channel, 0, Register, scram_save, 0xF);
  }
#endif // LPDDR4_SUPPORT
  return MMRC_SUCCESS;
}
...
)
{
  ...
  //
  //  Sweep RXVOC from 0-> 1 for all DQ, then Vref is good otherwise change Vref
  //
  ...
  )
{
  ...
  L_CpgcRunTest (MrcData, Channel, &ByteErrorStatus0, &BitErrorStatus[0], &BitErrorStatus[1], ErrorCount);
...
DeterminePassingVref1 (
  ...
  IN  INT16        Dim1StartPoint[MAX_TIMING][MAX_RANKS][MAX_CHANNELS][MAX_KNOBS][MAX_STROBES][2],
    ...
    for (Loop = 0; Loop < ControlKnobs[0].NumberOfTiming; Loop++) {
      for (Channel=0; Channel < MAX_CHANNELS; Channel++) {
        for (Rank = 0; Rank < MAX_RANKS; Rank++) {
          ...
          Mmrcmemcpy(ResultsV[Dim2Loop][Loop][Channel][Rank], Results[Loop][Channel][Rank],
            MAX_KNOBS * MAX_STROBES * MAX_BITS * 2 * 2); //Manuel 3 to 2
...
FinalCalculateCenter2 (
  ...
  for (Loop = 0; Loop <= (NumberSamples)/2; Loop++) {
    ...
    ( Param->FinalCalculateCenter2.ResultsV[NumberSamples/2 + Loop][Timing][Channel][Rank][Knob][Strobe][Bit][HIGH] + Param->FinalCalculateCenter2.ResultsV[NumberSamples/2 - Loop][Timing][Channel][Rank][Knob][Strobe][Bit][HIGH] +
      Param->FinalCalculateCenter2.ResultsV[NumberSamples/2 - Loop][Timing][Channel][Rank][Knob][Strobe][Bit][LOW]  + Param->FinalCalculateCenter2.ResultsV[NumberSamples/2 + Loop][Timing][Channel][Rank][Knob][Strobe][Bit][LOW]
    ));
        ...
        if (Param->ProgramSetGet.FirstEntryFlag == TRUE) {
          ...
          for (Knob= 0; Knob < MAX_KNOBS; Knob++) {
            for (Strobe = 0; Strobe < MAX_STROBES; Strobe++) {
              ...
              //
              Dim1StartPoint[Rank][Channel][Knob][Strobe][LOW]  = (UINT16) (Results[Timing][Channel][Rank][Knob][Strobe][0][LOW]);
              for (BitLoop=1; BitLoop<MAX_BITS; BitLoop++) {
                if (Results[Timing][Channel][Rank][Knob][Strobe][BitLoop][LOW] > Dim1StartPoint[Rank][Channel][Knob][Strobe][LOW]){
                  Dim1StartPoint[Rank][Channel][Knob][Strobe][LOW] = Results[Timing][Channel][Rank][Knob][Strobe][BitLoop][LOW];
                }
              }
              //
          ...
          //
          // PBD Low Limit comparison
          //
          while (Param->ProgramSetGet.AllDone == FALSE && Param->ProgramSetGet.ControlKnobs[0].PBStartValue >  0xF) {
...
//  UINT8     Channel;
  ...
  if (ControlKnobs[0].SharedIndex != -1) {
    NumberControlKnobs++;
    for (Loop = 0; Loop < NumberControlKnobs; Loop++) {
      ControlKnobs[Loop] = ControlKnobsBackup[Loop];
    }
    //
    // Must duplicate the results across all timings.
    //
    for (Sample=0; Sample < ControlKnobs[MrcData->NumberOfTiming].NumberSweeps; Sample++) {
      Mmrcmemcpy(ResultsV[Sample][1], ResultsV[Sample][0], MAX_CHANNELS * MAX_RANKS * MAX_KNOBS * MAX_STROBES * MAX_BITS * 2 * 2);
    }
...
CompleteSweep (
        ...
        for (Rank = 0; Rank < MAX_RANKS; Rank++) {
          if (RunOnThisChannel (MrcData, Channel, Rank)) {
            ...
            //NumStrobeLoops KIRKREDMAN
              Create1DSweepLastPass(MrcData, Channel, Rank, ControlKnobs[ControlIndex].Index, NumberKnobs, Dim1StartPoint[Rank], Low, High, ControlKnobs[ControlIndex].Step[1], FALSE, FALSE, TRUE, MrcData->Channel[Channel].MaxDq[Rank], GenericMemoryTest, ResultsTemp[Rank], "", 0);
            //
            // Set new starting points based on the results.
            //
            for (Knob= 0; Knob < MAX_KNOBS; Knob++) {
      ...
      for (Channel=0; Channel < MAX_CHANNELS; Channel++) {
        for (Rank = 0; Rank < MAX_RANKS; Rank++) {
          if (RunOnThisChannel (MrcData, Channel, Rank)) {
            ...
            //
            //NumStrobeLoops
            Create1DSweepLastPass (MrcData, Channel, Rank, ControlKnobs[ControlIndex].Index, NumberKnobs, Dim1StartPoint[Rank], Low, High, ControlKnobs[ControlIndex].Step[2], FALSE, FALSE, TRUE, MrcData->Channel[Channel].MaxDq[Rank], GenericMemoryTest, ResultsTemp[Rank], "", 1);
            //
            // Set new starting points based on the results.
            //
            for (Knob= 0; Knob < MAX_KNOBS; Knob++) {
  ...
  for (Channel=0; Channel < MAX_CHANNELS; Channel++) {
    for (Rank = 0; Rank < MAX_RANKS; Rank++) {
      if (RunOnThisChannel (MrcData, Channel, Rank)) {
          if ((ControlKnobs[ControlIndex].SharedFlags & KNOB_SHARED) == KNOB_SHARED) {
              Mmrcmemcpy(Results[Channel][Rank][0], ResultsTemp[Rank][Channel][0], MAX_STROBES*MAX_BITS * 2 * sizeof(UINT16));
              //Mmrcmemcpy(Results[Channel][Rank][0], ResultsTemp[Rank][Channel][0], MAX_STROBES*MAX_BITS * 2 * sizeof(UINT16));  // 1 knobs
          }
          else {
              Mmrcmemcpy(Results[Channel][Rank][0], ResultsTemp[Rank][Channel][0], MAX_STROBES*MAX_BITS * 2 * sizeof(UINT16));
              //Mmrcmemcpy(Results[Channel][Rank][0], ResultsTemp[Rank][Channel][0], MAX_STROBES*MAX_BITS * 2 * sizeof(UINT16));
  ...
  )
{
...
#if INSTRUMENTALGOS
  UINT8         SaveDebugMsgLevel;
  UINT64        TscInitial, TscFinal;    //kirk
#endif //INSTRUMENTALGOS
  ...
  for (Knob=0; Knob < NumberKnobs; Knob++)  {
    ...
    //GetSetDdrioGroup2 (MrcData, Channel, Rank, 0, TxVref , CMD_GET_REG, (UINT32 *)(&WrVValue));
    //GetSetDdrioGroup2 (MrcData, Channel, Rank, 0, TxCaVref, CMD_GET_REG, (UINT32 *)(&CaVValue));
    //MmrcDebugPrint ((MMRC_DBG_MIN, "C:%d R:%d CMD:%d CTL:%d WRV:%d CAV:%d\n", Channel, Rank, CmdValue, CtlValue, WrVValue, CaVValue));
   ...
   if (AllBitsPassed == FALSE) {
     //Store the current cmd and ctl value and set the ect trained value for cmd and ctl for dram reset
      ...
      GetSetDdrioGroup2 (MrcData, Channel, Rank, 0, CmdAll, CMD_SET_VAL_FC, &CmdCacheValue);
      GetSetDdrioGroup2 (MrcData, Channel, Rank, 0, CtlAll, CMD_SET_VAL_FC, &CtlCacheValue);
#if INSTRUMENTALGOS
      SaveDebugMsgLevel = MrcData->MrcDebugMsgLevel;
      MrcData->MrcDebugMsgLevel = 0;
      LCTJedecResetCount++;
...
#if INSTRUMENTALGOS
      TscFinal = ReadTsc();
      LCTJedecResetTimeTotal += (TscFinal - TscInitial);
      MrcData->MrcDebugMsgLevel = SaveDebugMsgLevel;
#endif //INSTRUMENTALGOS
      //MmrcDebugPrint ((MMRC_DBG_MIN, "%d %d Cmd Ctl PI = %d, %d\n", Channel, Rank, CmdCacheValue, CtlCacheValue));
      //write back the cmd and ctl value to reg after jedec init because the idsweep is using CMD_SET_OFFSET that reads from reg
      GetSetDdrioGroup2 (MrcData, Channel, Rank, 0, CmdAll, CMD_SET_VAL_FC, (UINT32 *)&(CmdValue));
      GetSetDdrioGroup2 (MrcData, Channel, Rank, 0, CtlAll, CMD_SET_VAL_FC, (UINT32 *)&(CtlValue));
      //MmrcDebugPrint ((MMRC_DBG_MIN, "%d %d Cmd Ctl PI = %d, %d\n", Channel, Rank, CmdValue, CtlValue));
      //GetSetTxCaVref (MrcData, 0, Channel, 0, Rank, 0, 0, 0, 0, CMD_GET_REG, &Value);
      //GetSetTxCaVref (MrcData, 0, Channel, 0, Rank, 0, 0, 0, 0, FC_WR, &Value);
...
void CalculateMasterFloorplan(MMRC_DATA * MrcData, int Channel)
{
  ...
  {
    MrcData->MasterFloorPlan[MrcData->CurrentBlueprint][Channel][bl].Strobelane = FloorPlan[Channel][MrcData->Channel[Channel].Dimm2DunitReverseMapping[bl*8]/8].Strobelane;
    MrcData->MasterFloorPlan[MrcData->CurrentBlueprint][Channel][bl].Channel = FloorPlan[Channel][MrcData->Channel[Channel].Dimm2DunitReverseMapping[bl*8]/8].Channel;
  }
}
/*
...
{
  UINT8 *Dimm2DunitMapping = (UINT8 *) MrcData->Channel[Channel].Dimm2DunitMapping;  // Single-dimensional array.  The index into the array is the swizzled location.  The array value is the unswizzled destination
  UINT32 Swizzled = 0;
  UINT8 CurrBit;
...
{
  UINT8 *Dimm2DunitReverseMapping = (UINT8 *) MrcData->Channel[Channel].Dimm2DunitReverseMapping;  // Single-dimensional array.  The index into the array is the swizzled location.  The array value is the unswizzled destination
  UINT32 Unswizzled = 0;
  UINT8 CurrBit;
...
)
{
  ...
  UINT8 *Dimm2DunitReverseMapping = (UINT8 *) MrcData->Channel[Channel].Dimm2DunitReverseMapping;
...
{
#if 0
  UINT8  HitZero[MAX_STROBES];
  UINT8  HitOne[MAX_STROBES];
  UINT8  Strobe, Bit;
  UINT32 TempValue;
  UINT8  InitialVref, NominalVref, VrefStep;
  UINT32 Vref;
  UINT8  StrobePass[MAX_STROBES];
  UINT8  StrobeState[VOC_VREF_CNT][MAX_STROBES];
  UINT8  StrobeVref[MAX_STROBES];
  UINT8  BitVoc[MAX_BITS][MAX_STROBES];
  UINT8  StrobePassCount;
  UINT32 OneSample, ZeroSample, XorSample;
  UINT32 Mask;
  UINT8  Count, BitFailCount;
  UINT8  Voc;
  UINT8  i;
  UINT32 SampleAnd;

  StrobePassCount = 0;
  NominalVref     = 0x10; //Get Value from Hari V
  VrefStep        = 1;
  InitialVref     = NominalVref - (2 * VrefStep) /* Lowest Vref we are testing */;

  MmrcMemset (HitZero , 0, MAX_STROBES /* MAX_BITS */ * sizeof (UINT8));
  MmrcMemset (HitOne , 0, MAX_STROBES /* MAX_BITS */ * sizeof (UINT8));
  MmrcMemset (StrobePass , 0, MAX_STROBES /* MAX_BITS */ * sizeof (UINT8));
  MmrcMemset (StrobeState , 0, VOC_VREF_CNT * MAX_STROBES /* MAX_BITS */ * sizeof (UINT8));

  for (Strobe =0; Strobe < MrcData->Channel[Channel].MaxDq[0]; Strobe++)
  {
    /* Setup DiffAmp and ODT */
    //Override Enable Bit
    TempValue = 1;
    GetSetDdrioGroup2 (MrcData, Channel, 0, Strobe, RxDiffampOvrDq, CMD_SET_VAL_FC_UC, &TempValue);
    GetSetDdrioGroup2 (MrcData, Channel, 0, Strobe, RxDiffampOvrDqs, CMD_SET_VAL_FC_UC, &TempValue);
    GetSetDdrioGroup2 (MrcData, Channel, 0, Strobe, RxOdtOvr, CMD_SET_VAL_FC_UC, &TempValue);
    //Override bit
    TempValue = 1;
    GetSetDdrioGroup2 (MrcData, Channel, 0, Strobe, RxDiffampValDq, CMD_SET_VAL_FC_UC, &TempValue);
    GetSetDdrioGroup2 (MrcData, Channel, 0, Strobe, RxDiffampValDqs, CMD_SET_VAL_FC_UC, &TempValue);
    GetSetDdrioGroup2 (MrcData, Channel, 0, Strobe, RxOdtVal, CMD_SET_VAL_FC_UC, &TempValue);
  }
  MmrcDebugPrint ((MMRC_DBG_MAX, "Finding Optimal Vref\n"));
  for (Count = 0; Count < VOC_VREF_CNT; Count++)
  {
    Vref = InitialVref + (Count * VrefStep);
    MmrcDebugPrint ((MMRC_DBG_MAX, "Vref:0x%04d  ", Vref));
    for (Strobe =0; Strobe < MrcData->Channel[Channel].MaxDq[0]; Strobe++)
    {
      if (StrobePass[Strobe] == 1) continue; //If Strobe Passes
      HitZero[Strobe] = 0;
      HitOne[Strobe] = 0;
      BitFailCount = 0;
      TempValue = Vref;
      if (!IsStrobeValid (MrcData, Channel, 0, Strobe))
      {
        ...
        //  Program Vref
        //
        GetSetDdrioGroup2 (MrcData, Channel, 0, Strobe, RxVref, CMD_SET_VAL_FC_UC, &Vref);
        //
        //  Enable Voc
        //
        TempValue = 1;
        GetSetDdrioGroup2 (MrcData, Channel, 0, Strobe, RxVocEnDq, CMD_SET_VAL_FC_UC, &TempValue);
        //
        //  Set Voc Offset to Max
        //
        TempValue = VOC_MAX;
        GetSetDdrioGroup2 (MrcData, Channel, 0, Strobe, RxVocVal, CMD_SET_VAL_FC_UC, &TempValue);
        //
        //  Sample the VOC Feedback and count 1's
        //
        GetSetDdrioGroup2 (MrcData, Channel, 0, Strobe, RxVocSmp, CMD_GET_REG, &OneSample);
        for (Bit=0; Bit < MAX_BITS; Bit)
        {
          Mask = (1 << Bit);
          if ((OneSample & Mask) != 0)
        ...
        //
        // Set Voc Offset to Min
        //
        TempValue = VOC_MIN;
        GetSetDdrioGroup2 (MrcData, Channel, 0, Strobe, RxVocVal, CMD_SET_VAL_FC_UC, &TempValue);
        //
        //  Sample the VOC Feedback and Count the 0's
        //
        GetSetDdrioGroup2 (MrcData, Channel, 0, Strobe, WrLvlSmp, CMD_GET_REG, &ZeroSample);
        XorSample = OneSample ^ ZeroSample;
        for (Bit=0; Bit < MAX_BITS; Bit)
        {
          Mask = (1 << Bit);
          if ((ZeroSample & Mask) == 0)
            HitZero[Strobe]++;
          //
          //  If the Low Bit is 1 or the low and high bit are the same, we count it as an error
          //
          if ((XorSample & Mask) == 0 || (ZeroSample & Mask) != 0)
            BitFailCount++;
        }
        //
        //  If we hit 0's and 1's in all the bits, then we are done for this strobe
        //  Else we track the number of strobes where the Min and Max Samples are the same and
        //
        if ((HitZero[Strobe] == MAX_BITS) && (HitOne[Strobe] == MAX_BITS))
        {
          StrobeState[Count][Strobe] = MAX_BITS;
          StrobePass[Strobe] = 1;
          StrobeVref[Strobe] = Count;
          StrobePassCount++;
          MmrcDebugPrint ((MMRC_DBG_MAX, "%02dP  ", MAX_BITS));
        }
        else
        {
          StrobeState[Vref-InitialVref][Strobe] = BitFailCount;
          MmrcDebugPrint ((MMRC_DBG_MAX, "%02dF  ", MAX_BITS-BitFailCount));
        }
      }
    }
    MmrcDebugPrint ((MMRC_DBG_MAX, "\n"));
  ...
  //
  // Check the condition where not all bytelanes dont have passing Vrefs
  //
  if (StrobePassCount < MrcData->Channel[Channel].MaxDq[0])
  {
    for (Count = 0; Count < VOC_VREF_CNT; Count++)
    {
      for (Strobe=0; Strobe < MrcData->Channel[Channel].MaxDq[0]; Strobe++)
      {
        if (StrobePass[Strobe] != 1)
        {
          //
          //  If there are more passing bits in the current Vref vs what is stored for the Current BL, then select the current Vref
          //
          if (StrobeState[Count][Strobe] > StrobeState[StrobeVref[Strobe]][Strobe])
          {
            StrobeVref[Strobe] = Count;
          }
        }
  ...
  //
  // Program the Vrefs for all the BLs where we didn't pass
  //
  MmrcDebugPrint ((MMRC_DBG_MAX, "Final Vrefs\n"));
  for (Strobe=0; Strobe < MrcData->Channel[Channel].MaxDq[0]; Strobe++)
  {
    Vref = InitialVref + (StrobeVref[Strobe] * VrefStep);
    MmrcDebugPrint ((MMRC_DBG_MAX, "%04d  ", Vref));
    if (StrobePass[Strobe] != 1)
    {
      GetSetDdrioGroup2 (MrcData, Channel, 0, Strobe, RxVref, CMD_SET_VAL_FC_UC, &Vref);
    }
  ...
  //
  //  Find Transitions
  //   Assumption: Voc0 should be 0 and VocF should be 1 in an ideal state
  //
  for (Voc = VOC_MIN; Voc < VOC_MAX; Voc++)
  {
    for (Strobe=0; Strobe < MrcData->Channel[Channel].MaxDq[0]; Strobe++)
    {
      SampleAnd = 0xFF;
      ...
      //  Sample the feedback
      //
      for (i=0; i < VOC_SAMPLES; i++)
      {
        GetSetDdrioGroup2 (MrcData, Channel, 0, Strobe, WrLvlSmp, CMD_GET_REG, &TempValue);
        SampleAnd &= TempValue;
      }
      for (Bit=0; Bit < MAX_BITS; Bit++)
      {
...
UINT8 SwizzleBytelane(MMRC_DATA * MrcData, UINT8 ch, UINT8 bl, UINT8 InData)
{
  ...
  UINT8   odata=0;
  if (ch<MAX_CHANNELS)
    Mapping = MrcData->Channel[ch].Dimm2DunitMapping;
  else
    return 0xCD;
  if (bl >= MAX_STROBES)
    return 0xFE;
  ...
  )
{
  ...
  //
  TimingParamVar = TimingParamHash (MrcData->TimingParamsConfig, FALSE);
  for (i = 0; i < TimingParamsUnder256Size; i++) {
    if (TimingParamsUnder256[i][0] == TimingType) {
      return TimingParamsUnder256[i][TimingParamVar];
    }
  }
  for (i = 0; i < TimingParamsOver255Size; i++) {
    if (TimingParamsOver255[i][0] == TimingType) {
      return TimingParamsOver255[i][TimingParamVar];
    }
  }
