...
#include "Spd.h"
#include "MmrcParamsSaveRestore.h"
...
#define STEPPING_E1   0x11
#define STEPPING_E2   0x12
#define STEPPING_E_C0 0x13
#define STEPPING_XA0  0x20
#define STEPPING_P0   0x40
#define STEPPING_P_B0 0x50

#define SERIES_BXT    0x00
#define SERIES_BXT_1  0x01
#define SERIES_BXT_X  0x02
#define SERIES_BXT_P  0x03
...
/*
CHANNEL_VOLATILE - Volatile Channel Specific parameters
*/
//typedef struct {
//  UINT32                    CachedValues[TOTAL_NUM_ELEMENTS + 1][MAX_RANKS][MAX_STROBES];
...
typedef struct {
  ...
  UINT16    BoardId;
  UINT8     Reserved[1]; // keep 4-byte alignment
  } MrcInput;
...
typedef struct {
  ...
  UINT8                     NumberOfChannels[TypeDdrT];
...
//  UINT8                     InputDdrVoltage;
 ...
 // UINT32                    ControlValue;
  ...
  UINT8                     MaxBits;
//  UINT32                    RestoreCmdCtrlValue[MAX_CHANNELS][MAX_TIMING];
//  UINT32                    RestoreCtrlValue[MAX_CHANNELS];
...
//Rx Power Training
typedef enum {
  UNIQUE       = 1, // Single Register Instance Per Channel
  STROBE_BASED = 2, // Loop on Strobes
  RANK_BASED   = 4, // Loop on Ranks
  BIT_BASED    = 8  // Loop On Bits
} ACCESS_TYPE;
