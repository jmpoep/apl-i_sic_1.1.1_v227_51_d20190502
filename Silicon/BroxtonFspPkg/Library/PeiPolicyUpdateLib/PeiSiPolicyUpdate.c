...
{
  DEBUG ((DEBUG_INFO, "FspUpdatePeiSiPolicy() - start\n"));
  //
  // Update Silicon Policy Config
  //
  SiPolicy->EcPresent = 1;
  SiPolicy->OsSelection  = FspsUpd->FspsConfig.OsSelection;
  SiPolicy->DptfEnabled  = FspsUpd->FspsConfig.DptfEnabled;
  SiPolicy->PWMEnabled   = FspsUpd->FspsConfig.PWMEnabled;
  DEBUG ((DEBUG_INFO, "FspUpdatePeiSiPolicy() - end\n"));
  return EFI_SUCCESS;
}
